...
**不想留的話，自己清那個目錄**；今天沒有「不落盤」的開關。dsh 的關法是在部署設定裡拿掉那個 plugin，而我們清單上
`#settings/session-persistence` 那一列只講批次窗口，關掉它不會停止落盤，所以這條路還沒有等價物。
...
不會讓標題不再被裁切、cookie 不再過期、交付檔不再有上限、落盤不再批次、工具結果不再被截、
`--live` 不再有連線設定——那一列
