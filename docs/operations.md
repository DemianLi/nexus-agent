...
`@nexus/wire` 的一頁歷史上限是 8 MB，那個數字是「80 × 每張卡 (50000 ＋ 50000)」算出來的——讀作「一頁
最多裝 80 張滿版工具卡」，一張卡是結果文字加上給專屬卡的結構化資料（`meta`），兩者的上限都是這一格
（[#617](https://github.com/DemianLi/nexus-agent/issues/617)）。那個關係由 `apps/harness` 的一條測試釘著，
