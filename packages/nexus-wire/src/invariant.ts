 ...
 *
 * @returns 掛著它的條目，註冊 `@nexus/wire` 配套入口的 plugin。
 */
export function createWireInvariantPlugin(): PluginEntry {
  return { plugin: wireInvariantPlugin };
}
 ...
 */
export const wireInvariantPlugin: NexusPlugin = {
  name: 'wire-invariant',
  apply(registry) {
