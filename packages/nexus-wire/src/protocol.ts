 ...
 * ## 這個數字怎麼來的
 *
 * `8_000_000` 當初是 **80 × 每張工具卡的上限**，讀作「一頁最多裝 80 張滿版工具卡」。一張卡的上限是結果
 * 文字 50000 bytes 加上給專屬卡的 `meta` 50000 bytes——`meta` 的上限就是文字的上限
 * （[#617](https://github.com/DemianLi/nexus-agent/issues/617) 決定 2；在那之前一張卡只有文字，這個數字是
 * `4_000_000`）。80 這個倍數對著量測挑：典型的一頁是 0.20 MiB 量級（量的時候還沒有 `meta`，一頁讀檔也
 * 還是 100 行，今天的典型值會比它大，沒有重量過），當時離這裡有一個量級的餘裕，所以正常瀏覽一次都不會
 * 被切（切了只會讓分頁變碎，對所有人都變差）；而上面那兩個病態形狀會落回單位數 MiB 的兩倍。
 *
 * **讀檔卡現在比那個大**（[#630](https://github.com/DemianLi/nexus-agent/issues/630)）：讀檔一頁照 dsh 放大到
 * 2000 行之後，讀檔 `meta` 的上限放寬成文字的**兩倍**，所以一張讀檔卡最壞是 150000 bytes，一頁約裝 53 張；
 * 搜尋與 diff 仍是 100000、80 張。8 MB 沒跟著改（demian 2026-09-25 拍板）：這個上限本來就是軟的（見下面），
 * 最壞的卡少裝幾張只是讓分頁碎一點。
 *
 * **與那些數字的關係由 `apps/harness` 的絆索釘著**（每則上限與讀檔 `meta` 的倍數住在 app 裡，wire 不能往上
 * import，所以這裡只能寫字面值）：那條測試把 8 MB、50000 與兩倍三個值一起釘死，任何一個動了都會紅。
 * 同一個做法見 `conversation.ts:927` 那條。
 *
 * **但它只釘得住出廠那一份**（[#538](https://github.com/DemianLi/nexus-agent/issues/538)）：每則上限
 * 在 #538 之後是清單上 `tool-text` 那一列的 `config`，**部署在 patch 裡改掉它，這個比例就不再成立，
 * 而且沒有任何東西會紅**。配對的另一半是這裡——一個協定常數，兩端對「一頁多大」的共識不該是執行期
 * 的事，所以它刻意沒跟著變成設定。
 *
