 ...
 *
 * 回聲工具的輸出是純函式結果，`index.test.ts` 直接驗；工具名撞名歸
 * `PluginRegistry` 的註冊期擋（同一個 `echo` 名字註冊兩次當場拋）。這個套件真正扛的那條
 * 保護——**沒有一行 import `@nexus/harness`**——來源是 pnpm 的相依隔離加 typecheck gate，
 * 真的寫了那行 `tsc` 會以 `TS2307` 擋下（理由見 `index.ts` 檔頭）。三樣都是 dsh 明說的
 * 「型別、載入或單元測試關注點」。
 *
 * @module
 */
...
/** 這個配套入口認領的 package 名。 */
export const ECHO_INVARIANT_PACKAGE = '@nexus/plugin-echo';
...
/**
 * 把 `@nexus/plugin-echo` 的配套入口掛上去。
 *
 * 掛了它**不會裝上任何檢查**，唯一的作用是**保留包名歸屬**：`register()` 就算在
 * installer 是空的時候也把名字佔住，兩個 plugin 不會靜默認領同一個包名。
 *
 * @returns 掛著它的條目，註冊 `@nexus/plugin-echo` 配套入口的 plugin。
 */
export function createEchoInvariantPlugin(): PluginEntry {
  return { plugin: echoInvariantPlugin };
}
 ...
 */
export const echoInvariantPlugin: NexusPlugin = {
  name: 'echo-invariant',
  apply(registry) {
