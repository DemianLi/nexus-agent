 ...
 *
 * 連線、握手與工具清單抓取都是 IO 效果，由 `index.test.ts` 起真的 MCP
 * 子行程證（判準照 dsh 政策 4）；工具名前綴與撞名的規則整條歸 `names.ts` 自己擁有，
 * 而且它是純函式，有自己的測試。IO 效果與純函式結果都不是運行時不變量。
 *
 * @module
 */
