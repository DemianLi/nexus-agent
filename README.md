...
**難題加進來之後，判準終於在階梯上量得出 1.00 以下。** 舊的三條題目上五個橫階全部 `1.00`，
總參數量差 27 倍也分不出高下；換成新的四條之後，五階裡有四階掉到 1.00 以下。
**但方向還定不了**：`gpt-oss` 那道是 `0.94` → `1.00`（參數正確性），`nemotron-3` 那道是
`0.92` / `0.97` / `0.89`，不單調 —— 兩道的取樣都只有 6 次，所以「尺寸造成了那個差異」
**還沒被確立**，被確立的只有「判準不再飽和」。順帶一個跟尺寸無關的結果：
