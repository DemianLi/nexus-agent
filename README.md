...
**難題加進來之後，飽和解除了一半。** 舊的三條題目上五個橫階全部 `1.00`，總參數量差 27 倍
也分不出高下；換成新的四條之後，`gpt-oss` 那道階梯**內部**第一次量得出尺寸效應
（20B → 120B：參數正確性 `0.94` → `1.00`、多叫次數 `1.38` → `0.29`），而 `nemotron-3`
那道仍然量不出來（`0.92` / `0.97` / `0.89`，不單調）。順帶一個跟尺寸無關的結果：
