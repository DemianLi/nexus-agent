...
**跑出來的結果分兩半。** 五個橫階的工具呼叫成功率與參數正確性全部是 `1.00` —— 總參數量
從 20B 到 550B 差 27 倍，分不出高下。判準對照則是 `0.67` 與 `0.19`，所以**不是題目太淺**，
是**崩塌點在 20B 底下**。順帶一個跟尺寸無關的結果：gpt-oss 那一家平均 5071–5452 token、
單題 1–7 秒，Nemotron 那一家 7989–8767 token、最慢 35.6 秒 —— **成本跟尺寸無關，跟配方有關**，
最小的 `nano` 比最大的 `ultra` 還貴。

驅動器把**失敗與零分分開**：模型叫不出工具是 0 分（有資料），端點回 4xx 或掛住是
「沒有資料」，後者不會被平均進通過率。判準對照那一列的 `評到分 6 次` 就是這樣來的 ——
另外三次是端點回 `400 "This model only supports single tool-calls at once!"`，
拒的是平行工具呼叫，那不是分數。
