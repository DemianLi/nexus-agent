...
`--cases` 只跑指定的題目。**成本是題數 × 階數 × 取樣數的乘積**，跑滿是半小時級，
...
**結論是「沒有尺寸效應」，而且那是有效力的否定。** 舊的三條題目上五個橫階全部 `1.00`，
分不出高下；換成加難過的題目、取樣撐到 12 次之後，五階的參數正確性落在 `0.88`–`0.98`、
全距下探到 `0.33` —— **判準在每一階上都還有量程，只是尺寸沒有在那個量程上動**。
`gpt-oss` 那道從 `0.94 → 1.00`（n=6）縮回 `0.91 → 0.92`（n=12），`nemotron-3` 那道
（`0.92` / `0.98` / `0.88`）兩輪都不單調，而且 550B 那階兩次都是最低。

**所以選型落回成本那一欄**，而三個非品質的軸都指向同一個：`openai/gpt-oss-120b` ——
參數正確性 `0.92`（並列第二）、token 最省（8519，比 Nemotron 那一家少四到五成）、
多叫次數最低（`0.33`）。跨階梯讀這條線對**選型**合法，對**尺寸效應**不合法。
順帶：**成本跟尺寸無關，跟配方有關** —— 最便宜的一階是 120B 的 `oss-120b`，
而最小的 `nano` 比最大的 `ultra` 還貴。
