/*
 * 照搬 dsh `packages/client/ui-primitives/src/markdown/incremental.ts`（本機 clone `ddefc45`，MIT，
 * Copyright (c) DeepSeek；授權全文見 https://github.com/deepseek-ai/deepseek-harness/blob/master/LICENSE）。
 * 只改了格式（prettier）；行為不動。串流中凍結前面的 block、只重解最後兩個，外加未關閉 fence 的第二道邊界。
 */
