/*
 * 照 dsh `packages/client/ui-primitives/src/markdown/mathCompatibility.ts`（本機 clone `ddefc45`，MIT，
 * Copyright (c) DeepSeek）逐行搬來，只有排版與註解不同。`micromark-extension-math` 只認 `$`；模型常寫
 * `\(…\)`、`\[…\]` 與同一行的 `$$…$$`，這裡用同一套 token 補上，交給 `math()` 編成一樣的 math 節點。
 */
...
/* eslint-disable @typescript-eslint/no-this-alias -- micromark 只在外層 callback 綁 tokenizer 的 this。 */
