 ...
 * | 其他／斷線 | `'error'` | 是 |
 *
 * **沒有 `'binary'`。** 422（`not-text`）只住在預覽那條路裡（`readDeliverablePage` 掃 NUL），
 * 下載那條根本不看內容——二進位正是它存在的理由。把預覽的五態原封不動抄過來的話，會多一個
 * 永遠不會發生的分支，而那讀起來像「下載也可能被二進位擋住」。
 *
 * **下載的 413 跟預覽的 413 不是同一件事**，這是這一刀唯一會讓人搞錯的地方：
 * 預覽的 413 有兩個成因（整檔 > 32 MiB，或單頁 > 2 MiB），下載只吃前者。所以一個「太大不能預覽」
 * 的檔多半下載得下來（單頁超標的那一半），而真的撞上下載 413 時就沒有下一步了。
 *
 * @module
 */
