/**
 * 來源：shadcn registry `dialog`（style new-york，https://ui.shadcn.com/r/styles/new-york-v4/dialog.json），
 * shadcn CLI 4.21.0 `shadcn add message-scroller message bubble dialog sonner --overwrite`。裝進來就是我們的原始碼，
 * 不靠重跑 `shadcn add` 更新。
 * 改過的地方（照原型 tag `proto-375-design-language`）：border＋`shadow-lg` 換成 `bg-card`＋`shadow-menu`（§5）、
...
function DialogOverlay({
  ...
  return (
    <DialogPrimitive.Overlay
      ...
      className={cn(
        'fixed inset-0 z-50 bg-black/50 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:animate-in data-[state=open]:animation-duration-200 data-[state=open]:fade-in-0',
        className,
      )}
      {...props}
    />
