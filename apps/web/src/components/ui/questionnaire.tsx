/**
 * 來源：shadcn registry `questionnaire`（style radix-vega，https://ui.shadcn.com/r/styles/radix-vega/questionnaire.json；
 * new-york 沒有這一顆，§4.1 第 6 步），primitive 是 `@shadcn/react@0.3.1` 的 `questionnaire`。它的 `registryDependencies`
 * 有 `button`，`shadcn add --overwrite` 會把改過的 `button.tsx` 蓋回去，所以取 registry JSON 的檔案內容、照 CLI 的
 * 轉換改 import（`@/registry/radix-vega/ui/button` → `@/components/ui/button`、`IconPlaceholder` → lucide `CheckIcon`）。
 * 裝進來就是我們的原始碼，不靠重跑 `shadcn add` 更新。
 * 改過的地方（照原型 tag `proto-375-design-language`）：選項與輸入框的 `border`＋`shadow-xs` 換成 `bg-card shadow-material`、
 * 選中改 `bg-chip`、invalid 改內陰影（§5）；拿掉 `cn-font-heading` 與快捷鍵徽章的 `shadow-xs`；按鈕預設字換中文（§8）。
 */
