...
export interface MeasuredModel extends ModelUnderTest {
  /**
   * 最後一次走完整基準任務量它的日期，`YYYY-MM-DD`。
   *
   * **這份清單說的是「量過」，不是「現在還叫得動」，而後者沒有任何測試守得住** ——
   * 守得住就要憑證。三輪盤點掉了 5 個可用的模型（14 → 16 → 9），`openai/gpt-oss-120b`
   * 是量過、選上、然後下架的。所以日期舊了就當成**要重探**：先照檔頭那套盤點方法打一次，
   * 再拿這一行的數字去比。`live-model.ts` 的 `modelGoneMessage` 是它下架時的那句話。
   */
 ...
 * 2026-08-29 那輪選型調查（294 次執行）裡它跑了七題各三次，**參數正確性 0.33（七題）／
 * 0.53（難題），14/21 評到分** —— 離 `1.00` 很遠，判準在真實執行下有量程。這就是它的全部作用。
 *
 * **同一輪的 `meta/llama-3.2-90b-vision-instruct` 拿到 0.53 / 0.67，而那不是一條尺寸的線。**
 * 兩個模型之間分得出高下，證明的是判準夠利；讀成「大的比較好」需要一道階梯，而階梯
 * 收掉了（見檔頭）—— 何況那一對兩邊都有三分之一的執行沒評到分。90B 現在六次探測全逾時，
 * 這一對也重跑不了了。
 */
