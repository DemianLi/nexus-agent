 ...
 * | 型別 | 在哪 | 回答什麼 |
 * | --- | --- | --- |
 * | {@link ModelTier} | `tiers.ts` | 同家族內「只有尺寸在變」時分數怎麼動 |
 * | `SCORER_CONTROL` | `tiers.ts` | 判準量不量得出 `1.00` 以下 |
 * | `SurveyModel` | `survey.ts` | 選型：這把 key 上叫得動的模型，同題表現如何 |
 *
 * **第三個沒有尺寸欄位，而那是設計而不是偷懶**（[#85](https://github.com/DemianLi/nexus-agent/issues/85)
 * 第 3 條）：十六個候選來自不同廠商、不同訓練配方，那條線讀不成尺寸效應。把欄位拿掉，
 * 「不是在比尺寸」就從報告裡的一句話變成型別上的事實 —— 想照尺寸排序的人編不過。
 */
