 ...
 * 其中 `ds-pro` 後來單獨、每次隔 60 秒重跑到 **21/21 零失敗** —— 它缺的是 `throttled`，
 * 也就是我們打太快，不是它跑不完。
 *
 ...
 * 全部是純等待**。**逾時不在 `retryDecision` 的放棄清單裡，而且是刻意的**：#521 查過 dsh 的 `TIMEOUT`
 * 也在預設可重試碼裡，一次偶發的慢本來就該重試；分不出「這顆模型永遠逾時」是代價。要跑就先用
 * `--models` 排掉這五個。
 *
