...
describe('預設清單', () => {
  it('十七個配套入口都在，而且各自認領自己的包名', async () => {
    // #107 拍的是「全進」。少掛的那幾個會讓「這個 package 沒有可檢的關係」與
    // 「這個 package 的檢查沒掛上」在診斷裡長得一模一樣，所以這裡數的是**十七**。
    ...
    expect(DEFAULT_PLUGINS.filter((plugin) => plugin.name.endsWith('-invariant'))).toHaveLength(17);
  });
});
