/**
 * 把 [`docs/operations.md`](../../../docs/operations.md) 裡那道核准指令的 `--plugins` 值讀出來。
 *
 ...
 *
 * 承重的只有 `--plugins` 那一個值，不是指令全文——`serve` 那條多 `--port 0`，CLI 那條根本不是
 * `serve`。所以釘的是參數。
 *
 ...
 * @returns `--plugins` 後面那個值。
 */
export function parseDocumentedFixture(source: string): string {
  const matches = [...source.matchAll(/--plugins\s+(\S+)/gu)];

  if (matches.length === 0) {
    throw new DocumentedFixtureError(
      `${OPERATIONS_DOC} 裡找不到任何 \`--plugins <值>\`。「核准」那一節該有一道 ` +
        '`pnpm --filter @nexus/harness run serve --plugins <fixture>` 的指令；' +
  ...
  if (matches.length > 1) {
    ...
    throw new DocumentedFixtureError(
      `${OPERATIONS_DOC} 裡有 ${String(matches.length)} 處 \`--plugins\`（${found}），` +
