...
本章處理多輪對話中的狀態怎麼表示、怎麼逐輪更新、在長互動裡怎麼維持，範圍從任務型對話的 DST 與資料集、LLM 時代的 zero/few-shot DST，到 LLM agent 的外部記憶與多輪退化 [arXiv:1810.00278][arXiv:2203.08568][arXiv:2310.08560][arXiv:2505.06120]。本節點精讀 24 篇、計入 23 篇，另有 7 篇候補未讀；計入的分佈是傳統任務型對話 DST 與資料集 9 篇（配額 9）、LLM 時代的 zero/few-shot DST 7 篇（配額 7）、LLM agent 的多輪狀態維護與多輪退化 7 篇（配額 8，少的那一篇 Drift No More 精讀後判為低價值，只當反例）（篇數由 reading-status.json 計算）[arXiv:2510.07777]。

全章的主軸是「狀態怎麼跨輪更新」：三個子領域發展出插值、覆寫、每輪重算、差分累加、只增不刪、agent 自己改寫、ADD／UPDATE／DELETE、讀取時裁決等做法 [arXiv:1606.03777][arXiv:2005.02877][arXiv:2203.08568][arXiv:2504.19413][arXiv:2512.12818]。依各篇筆記，只有兩篇把「本輪該更新什麼」和「累積後的狀態」分開報：GLAD 在消融表同時列 turn goal 與 joint goal，IC-DST 的 Figure 5 逐輪畫出狀態變化的 JGA 與累積狀態的 JGA [arXiv:1805.09655][arXiv:2203.08568]。但在狀態追蹤上，沒有一篇做能把累積錯誤單獨分離出來的對照，例如改餵 gold 的上一輪狀態；T7 計入的 MT-Bench-101 對一般的多輪回覆做過金上下文與自身上下文的對照，對象不是狀態 [arXiv:2402.14762]；記憶系統則連 UPDATE、DELETE 這類操作本身的正確率都沒有量（撰寫本章時的歸納，範圍限本調研讀過的論文）[arXiv:2203.08568][arXiv:2504.19413]。

第二個子領域雖名為「LLM 時代」，七篇裡真正以提示驅動大型 LM 的只有 IC-DST 與 FnCTOD，其餘五篇是微調的前身 [arXiv:2203.08568][arXiv:2402.10466]。T1 另外讀過兩篇以提示或 priming 做 DST 的論文，在 T1 都判為 📖，本章只當脈絡（見〈LLM 時代的 zero/few-shot DST〉）[arXiv:2304.04256][arXiv:2008.06239]。

範圍列出的「指令在多輪中的保持」，計入的 23 篇裡沒有一篇直接量；最接近的 Lost in Conversation 量的是一條完整指令被拆成多輪逐片給出時的表現，不是早先給的指令在後面是否仍被遵守（撰寫本章時依筆記判斷）[arXiv:2505.06120]。本調研讀過但沒計入的論文裡，有兩篇碰到這件事，都只能當脈絡：Drift No More 的合成任務在逐輪加入衝突指令時，看模型有沒有守住一開始給的字數限制，但精讀判定那張表像人工構造的示意 [arXiv:2510.07777]；2304.04256 只質性地寫到，約 10 輪之後 ChatGPT 可能忘記第一輪給的 schema 與規則 [arXiv:2304.04256]。直接量這件事的論文沒有進任何節點的候選池，所以這一塊在本章是收錄缺口，不能讀成研究空白（見〈已知缺口與未讀〉與〈同一個 session 內的多輪退化與指令保持〉）。

**範圍說明。** 本章的結論只涵蓋下面這個範圍：

- **時間窗。** 依文獻表的 arXiv 年月，計入的論文最早是 2016 年 6 月的 NBT，最晚是 2025 年 12 月的 Hindsight；📖 的 Drift No More 是 2025 年 10 月 [arXiv:1606.03777][arXiv:2512.12818][arXiv:2510.07777]。正文另外引用的其他節點筆記也都落在這個時間窗內。
- **未讀的候補。** 本節點的七篇候補都沒有讀。依計劃，候補只在節點計入不滿二十篇時動用，本節點計入二十三篇，所以第三個子領域雖然比配額少一篇，也沒有觸發。下面依篩選時記下的子領域分組，每篇後面依篩選時記下的理由說明它原本要補什麼；傳統 DST 那一組底下的小標是撰寫者依理由再分的：
  - LLM agent 的多輪狀態維護、對話記憶與多輪退化：Recursively Summarizing Enables Long-Term Dialogue Memory in Large Language Models（arXiv 2308.15022，未讀），以遞迴摘要維護長對話記憶；MemTool: Optimizing Short-Term Memory Management for Dynamic Tool Calling in LLM Agent Multi-Turn Conversations（arXiv 2507.21428，未讀），在多輪對話中動態管理工具的短期上下文；Global-to-local Memory Pointer Networks for Task-Oriented Dialogue（arXiv 1901.04713，未讀），以記憶指標整合知識庫與多輪狀態。
  - 傳統任務型對話 DST 與資料集：
    - 架構：Scalable Multi-Domain Dialogue State Tracking（arXiv 1712.10224，未讀），不綁定 slot 值集合的多領域 DST；Dialog State Tracking: A Neural Reading Comprehension Approach（arXiv 1908.01946，未讀），把 DST 改寫成閱讀理解。
    - 低資源的遷移學習：MinTL: Minimalist Transfer Learning for Task-Oriented Dialogue Systems（arXiv 2009.12005，未讀）。
    - 考場本身：MultiWOZ 2.4: A Multi-Domain Task-Oriented Dialogue Dataset with Essential Annotation Corrections to Improve State Tracking Evaluation（arXiv 2104.00773，未讀），再修一次標註並重訂評估標準。IC-DST 的 Table 1（few-shot）與 Table 2（zero-shot 的逐領域 JGA）都同時報這一版與 2.1；multi-domain 的 zero-shot 35.3 與全部消融只做在這一版上（消融用 100 段 dev 對話）[arXiv:2203.08568]。D3ST 也有一部分結果報在這一版上 [arXiv:2201.08904]。
- **不涵蓋的部分。** 時間窗之後的論文、上面這幾塊，以及〈已知缺口與未讀〉列出的各塊，本章的結論都不涵蓋。本章寫「沒有一篇」「只有」的地方，除非另外註明，範圍都是本調研讀過的論文。
...
這九篇依序處理了幾件事：先讓模型不靠人工字典也能從語句認出 slot-value，再讓稀有值借用其他 slot 的資料，接著把問題放大到多領域並建立共同考場，然後把「值要事先列舉」與「slot 要事先列舉」兩層封閉假設逐層拆掉，最後回頭修改考場的標註與計分規則 [arXiv:1606.03777][arXiv:1805.09655][arXiv:1810.00278][arXiv:1905.08743][arXiv:1909.05855][arXiv:2007.12720]。貫穿全線、這九篇卻都沒有正面處理的，是跨輪的狀態到底怎麼維護（撰寫本章時的歸納，見本小節最後一段）。
...
**2020 年 7 月：回頭修尺（MultiWOZ 2.2）。** 在它之前已有一次修正：依 MultiWOZ 2.2 原文的轉述，MultiWOZ 2.1 修正了分佈在 40% 輪次中、32% 的對話狀態標註，並加上 slot 描述 [arXiv:2007.12720]。2.1 的論文本身不在本調研的語料裡（見〈已知缺口與未讀〉），而本章多數 JGA 都是在 2.1 上報的。MultiWOZ 2.2 指出 2.1 的標註仍有三類毛病 [arXiv:2007.12720]：
...
**本子領域的跨輪維護：每篇一種做法，各有失敗型態。** 把九篇放在一起看，跨輪維護的方式各不相同。下列七個模型的失敗型態，逐項核對筆記，都出自 limitations_observed，也就是精讀 agent 的分析。再逐篇核對 limitations_stated：除了 TripPy 在未來工作裡提到想加入部分遺忘的更新策略，其餘六篇的自述限制都沒有提到這些失敗型態；其中 SimpleTOD 自承多回合、長上下文的狀態追蹤只有定性範例，碰到的是「沒有被量」，不是截斷本身 [arXiv:2005.02877][arXiv:2005.00796]：

- NBT：固定係數的機率插值，沒被重提的目標會衰減掉（精讀時推算）[arXiv:1606.03777]。
- GLAD：規則覆寫，錯的值會一直留到同一 slot 被再次指定（精讀時分析）[arXiv:1805.09655]。
- TRADE：每輪從最近 l 回合的歷史重新生成，視窗外提過的值只能靠當輪重新找到（精讀時分析）[arXiv:1905.08743]。
- SUMBT：狀態藏在 RNN 隱藏狀態裡，不能檢查也不能外部修正（精讀時分析）[arXiv:1907.07421]。
- SGD 原型：累加差分，判錯的值會一路帶下去，也無法從更早的 turn 或別的服務 frame 複製值（精讀時分析）[arXiv:1909.05855]。
- TripPy：只覆寫、不刪除（精讀時分析；論文在未來工作中自承更新策略簡單，想加入部分遺忘的選項）[arXiv:2005.02877]。
- SimpleTOD：每回合從完整歷史重寫整份狀態，超過 1,024 token 時較早的約束會無聲消失，而這個風險沒有被量（精讀時分析；論文自承多回合、長上下文的狀態追蹤只有定性範例）[arXiv:2005.00796]。
- MultiWOZ 2.2 不提供模型，而是規定狀態值要能回溯到出處 [arXiv:2007.12720]。

這七篇模型論文都沒有把「跨輪維護」這一步單獨拿出來評估 [arXiv:1606.03777][arXiv:1805.09655][arXiv:1905.08743][arXiv:1907.07421][arXiv:1909.05855][arXiv:2005.02877][arXiv:2005.00796]。最接近的是 GLAD：它的消融表同時列 turn goal 與規則累加後的 joint goal，看得出累積會放大逐輪錯誤（見〈JGA 量的到底是什麼〉），但沒有「照 gold 累積」的對照，分不出 joint goal 的損失有多少是前面錯了一路帶下來 [arXiv:1805.09655]。資料面也一樣窄：MultiWOZ 的蒐集設計裡，明文引發目標變更的只有一種做法，就是讓初始約束查無資料、使用者改用替代值 [arXiv:1810.00278]。精讀時據此判斷這是唯一的觸發方式；群眾使用者有沒有自發改口、撤回條件，實際占多少輪次，本調研讀過的論文都沒有統計，本調研自己也統計不了（見〈程式驗證〉）[arXiv:1810.00278]。

### LLM 時代的 zero/few-shot DST（2020–2024）

本子領域七篇都被歸在「LLM 時代的 zero/few-shot DST」，但以提示或 in-context learning 驅動大型 LM 的只有 IC-DST 與 FnCTOD [arXiv:2203.08568][arXiv:2402.10466]。其餘五篇都是微調：TOD-BERT 從 BERT-base 繼續預訓練，T5DST 用 T5-small，TransferQA 用 T5-large，DaP 用 T5-small／base，D3ST 最大到 11B 的 T5 XXL [arXiv:2004.06871][arXiv:2105.04222][arXiv:2109.04655][arXiv:2109.07506][arXiv:2201.08904]。DaP 甚至沒有任何 zero-shot 或 few-shot 實驗 [arXiv:2109.07506]。所以這一段比較準確的讀法是：上一個子領域的考場與狀態定義，怎麼被搬到「少標註或不標註」的設定上。主軸有三條：狀態怎麼表示、怎麼向模型提問、上一輪的狀態有沒有被沿用。以提示驅動 LLM 的這條線只有兩篇計入，所以本子領域關於提示法的結論都很薄；候選池裡另有兩篇專講 ChatGPT 或 LLM 驅動 DST 的論文，以制式理由落選、沒有讀（見〈已知缺口與未讀〉）。

**T1 讀過、在本章只當脈絡的兩篇（📖）。** 兩篇都以提示或 priming 做 DST，在 T1 精讀後判為低價值，本章的主張不靠它們撐：

- 2008.06239 讓 GPT-2 不更新參數、只在前綴塞少量範例做 DST，joint accuracy 最高只有 3.5%（gpt2-large、15 shots）[arXiv:2008.06239]。精讀時指出，它只看最後一句使用者話、把非 None 的值逐輪累加，改口與刪除 slot 都處理不了 [arXiv:2008.06239]。IC-DST 說先前把 ICL 用在 DST 表現不如微調時，點名的是 Few-Shot Bot 與 UnifiedSKG，參考文獻裡沒有這一篇 [arXiv:2203.08568]。
- 2304.04256 讓 2023 年 1 月版的 ChatGPT 在同一個 session 裡逐輪送入對話、以先前的預測為基礎更新狀態，MultiWOZ 2.1 的 JGA 在 Table 3 是 60.28、在 Table 4 是 60.02；同一模型改成每輪另開 session 的單輪提示是 58.05 [arXiv:2304.04256]。精讀時指出三件事：兩表數字對不上又沒有變異量，+1.97 的差距穩不穩無從判斷；它勝過 GPT-3.5、Codex 約 26 分，混了模型與提示兩個變因；提示規則字面上禁止輸出先前出現過的屬性，照做就無法處理改口 [arXiv:2304.04256]。它的做法正落在本章「狀態怎麼跨輪更新」的主軸上：狀態留在同一段對話歷史裡逐輪更新，而不是像 IC-DST 那樣每輪重組提示 [arXiv:2304.04256][arXiv:2203.08568]。
...
測試時 IC-DST 沿用的是模型自己累積的預測狀態，不是 gold [arXiv:2203.08568]。這不是本章第一篇這樣做的：NBT、GLAD、SGD 原型與 TripPy 測試時都在自己的預測上累積 [arXiv:1606.03777][arXiv:1805.09655][arXiv:1909.05855][arXiv:2005.02877]。在本章計入的論文裡，IC-DST 的不同之處是把上一輪預測狀態的完整內容當成顯式文字輸入交給凍結的 LLM；TripPy 只以二元特徵標記哪些 slot 已填過，不帶值（撰寫本章時比對筆記）[arXiv:2203.08568][arXiv:2005.02877]。但「上一輪狀態當顯式輸入」本身在微調路線上更早就有：TripPy 的原文描述，SOM-DST 把上一輪對話與上一輪的對話狀態一起送進 BERT 預測本輪狀態 [arXiv:2005.02877]；DaP 公開程式碼的 sequential 變體也吃「上一輪狀態＋當輪對話」（精讀時發現）[arXiv:2109.07506]。SOM-DST 本身本調研沒讀（見〈已知缺口與未讀〉）。它的「只預測變化」也和 SGD 原型的差分式 user goal 同形，差別在 SGD 原型不看上一輪狀態 [arXiv:2203.08568][arXiv:1909.05855]。

IC-DST 也是本章計入的論文裡唯一按輪次畫出 JGA 的一篇。Figure 5 畫出每一輪的三條 JGA 曲線，內文沒有給數值：預測的狀態變化那條全程維持高檔；把預測的變化累積起來的完整狀態，與直接預測完整狀態的那條，都隨輪次下降，前者降得較慢 [arXiv:2203.08568]。論文自己的解釋是後段輪次的狀態 slot 較多，而每輪的變化都只有少數幾個 slot，並沒有歸因於錯誤傳遞 [arXiv:2203.08568]。精讀時指出，這張圖沒有「餵 gold 的上一輪狀態」與「餵預測的上一輪狀態」的對照，所以累積狀態的下降有多少來自錯誤一路帶下來，仍然分不出來 [arXiv:2203.08568]。主模型 code-davinci-002 已於 2023-03-23 下架，原設定無法重現 [arXiv:2203.08568]。
...
兩派都沒量到自己的弱點：累積派最多只到 IC-DST 的逐輪曲線，沒有能分離錯誤傳遞的對照；重算派沒按對話長度分析 JGA [arXiv:2203.08568][arXiv:2201.08904][arXiv:2105.04222]。
...
**這個子領域實際涵蓋什麼（名實說明）。** 節點範圍寫的是「多輪狀態維護、多輪表現退化、指令在多輪中的保持」三件事，但計入的論文分佈很不平均，讀者要照下面的定義讀這一節（撰寫本章時依計入清單清點）：

- 外部記憶的六篇，主要評的是跨 session 的事實回想（DMR、LongMemEval、LoCoMo，MemoryBank 則在自建的模擬多日對話上出探針題），Generative Agents 評的是沙盒裡的可信度 [arXiv:2304.03442][arXiv:2305.10250][arXiv:2310.08560][arXiv:2503.08026][arXiv:2504.19413][arXiv:2512.12818]。本章只取它們的「狀態怎麼更新」這一面，不評它們當長期記憶好不好用。
- 同一個 session 內的多輪退化，計入論文裡專門量它的只有 Lost in Conversation 一篇，IC-DST 另外附帶畫了逐輪 JGA；另一篇是 📖 的反例 [arXiv:2505.06120][arXiv:2203.08568][arXiv:2510.07777]。
- 指令在多輪中的保持，計入論文裡沒有一篇直接量；只有兩篇 📖 碰到邊（見〈同一個 session 內的多輪退化與指令保持〉）。這是本調研的收錄缺口，不是研究空白，可以補的論文列在〈已知缺口與未讀〉。

本調研的節點範圍另外寫明「與對話狀態無關的長期個人化記憶不收」。MemoryBank 與 RMM 仍留在本章，理由是兩篇各有一條明確的狀態更新規則（遺忘衰減、Add／Merge），而跨子領域表要比的正是更新規則；它們的個人化目標與陪伴對話的評估，本章不討論 [arXiv:2305.10250][arXiv:2503.08026]。

兩條線幾乎沒有交會：記憶系統量的是跨 session 的事實回想、文件問答或可信度，量多輪退化的論文則沒有測任何記憶系統 [arXiv:2304.03442][arXiv:2310.08560][arXiv:2504.19413][arXiv:2505.06120][arXiv:2512.12818]。沿時間往下讀，最清楚的一條軸仍是「狀態怎麼更新」；退化與指令保持的證據集中在本節最後的小節。
...
**2023 年 5 月：分層摘要加遺忘曲線（MemoryBank）。** 記憶分三層：帶時間戳記的逐輪原文、LLM 產生的每日與全域事件摘要、每日與全域使用者畫像 [arXiv:2305.10250]。記憶保留率借用艾賓浩斯遺忘曲線 R = e^(−t/S)，召回一次 S 加 1、t 歸零 [arXiv:2305.10250]。量化實驗只比三個掛了記憶的變體，沒有「不掛記憶」的對照組，也沒有消融 [arXiv:2305.10250]。精讀時依公開程式碼指出，遺忘公式在程式碼裡寫成 exp(-t/5*S) [arXiv:2305.10250]。這條主張要拆成兩半讀：程式碼那一行只來自精讀筆記的轉述，快取裡沒有程式碼，本章無法判定；算術那一半由 `verify/02-memorybank-forgetting.py` 證實，依運算順序它等於 e^(−t·S/5)，召回越多次（S 越大）反而忘得越快，與論文公式隨 S 遞增的方向相反（見〈程式驗證〉）。
...
**2025 年 4 月：逐輪抽取加四種操作（Mem0）。** Mem0 每收到一組新訊息就觸發一次抽取；每個候選事實與最相似的前 10 筆舊記憶並列，由 LLM 以 function call 選 ADD、UPDATE、DELETE、NOOP [arXiv:2504.19413]。基本版的 DELETE 是實體刪除；圖版 Mem0g 衝突時把舊關係標為失效而不刪除 [arXiv:2504.19413]。和 RMM 相比，寫入時機從 session 結束改成逐組訊息，動作集合多了明確的刪除 [arXiv:2503.08026][arXiv:2504.19413]。在本章計入的論文裡，這組動作和 IC-DST 狀態變化裡的「新增、刪除、改值」是同一種語法，只是作用對象從 slot 換成自然語言事實（撰寫本章時的對照）[arXiv:2203.08568][arXiv:2504.19413]。所以在本章裡，Mem0 這組操作最近的前例是 IC-DST；這只在本調研讀過的論文裡成立，傳統 DST 裡有一篇本調研沒讀的論文能檢驗這一句（見〈已知缺口與未讀〉的「傳統 DST 裡以上一輪狀態為輸入的路線」）。LOCOMO 上 Overall J 是 Mem0 66.88、Mem0g 68.44，全文脈絡 72.90 仍然最高；Mem0 的賣點在成本，p95 總延遲是 1.440 s 對 17.117 s [arXiv:2504.19413]。精讀時發現，抽取的精確率與召回率、UPDATE／DELETE 判斷的正確率都沒有量，也沒有任何消融 [arXiv:2504.19413]。更根本的是主基準本身：精讀 LoCoMo 原論文時發現，它的事件圖只會單調累積，沒有事實被更新或推翻的題目 [arXiv:2402.17753]。所以 Mem0 在 LoCoMo 上的分數，在原理上量不到它的招牌操作 UPDATE 與 DELETE 做得對不對（撰寫本章時的推論）[arXiv:2402.17753][arXiv:2504.19413]。
...
**2025 年 10 月（📖，只當反例）：漂移有界？** Drift No More 以測試模型相對參考模型的逐輪 KL 散度定義漂移，宣稱 τ-bench 使用者模擬器的 KL 沒有隨輪次無界成長 [arXiv:2510.07777]。精讀判為低價值，理由是它用來證明「有恢復力」的迴歸診斷，在完全沒有動態的 i.i.d. 雜訊下也會得到負的斜率；程式驗證證實了這一點，也發現 Table 6 的六列裡只有兩列正落在這個虛無模型的中心（見〈爭議〉）[arXiv:2510.07777]。
...
在本調研讀過的論文裡，這六種做法之間，在同一個基準、同一個評審下比過的只有一組：RMM 的 Table 1 在 LongMemEval 上，以同一個 Gemini-1.5-Pro 評審比了 RMM 與 MemoryBank，準確率是 RMM-GTE 70.4、MemoryBank 59.6；同一張表也在 MSC 上以 METEOR 與 BERTScore 比了兩者 [arXiv:2503.08026]。但這裡的 MemoryBank 是 RMM 以 LongMemEval 的程式碼庫重新實作的，比的是整個系統，不是更新規則本身；精讀時還指出，重新實作的預設檢索器是 Contriever，同為 Contriever 時 RMM 只多 61.2 − 59.6 = 1.6 點 [arXiv:2503.08026]。Mem0 的 LoCoMo 表另外列了 MemoryBank 與 MemGPT，但只有引自既有論文的 F1 與 B1，沒有 J [arXiv:2504.19413]。Hindsight 的 LoCoMo 表並列了 Mem0 與 Mem0-Graph，但這幾列照抄 Backboard 公布的分數，Hindsight 明寫它們是參考點、不是重跑的基線；原文沒寫這幾列用哪個評審，它們與 Mem0 論文逐格相同，撰寫者推論可能帶著 Mem0 的 gpt-4o-mini judge（見〈記憶子領域內的爭議〉）[arXiv:2512.12818][arXiv:2504.19413]。沒有一篇把更新規則單獨拿出來比，也沒有一篇量過自己更新操作的正確率 [arXiv:2310.08560][arXiv:2503.08026][arXiv:2504.19413][arXiv:2512.12818]。六種做法之外，Zep 以雙時間的時間知識圖分別追蹤事實何時有效、何時被記錄；這是依 Hindsight 的相關研究段落，Zep 本身的更新語意本調研沒有直接讀到（見〈記憶子領域內的爭議〉與〈已知缺口與未讀〉）[arXiv:2512.12818]。這和前兩個子領域的處境一樣：〈方法比較〉一節的跨子領域表，三個子領域合計列了十三種維護方式（不含沒有顯式狀態的那一列），量的一直是下游結果（撰寫本章時的歸納）。

### 同一個 session 內的多輪退化與指令保持

這一節把散在各處、量「同一個 session 內越往後越差」的證據放在一起。本章計入的論文只有 Lost in Conversation 與 IC-DST 碰到這件事；另外兩個節點計入的 MT-Bench-101（T7）與 CollabLLM（T1）各有一張逐輪的圖；兩篇 📖 只當脈絡（撰寫本章時依筆記清點）[arXiv:2505.06120][arXiv:2203.08568][arXiv:2402.14762][arXiv:2502.00640]。

| 論文 | 怎麼量 | 對照 | 看到什麼 | 論文怎麼解釋 | 出處 |
| --- | --- | --- | --- | --- | --- |
| Lost in Conversation（本章計入） | 同一份指令一次給齊、條列成單輪、逐輪揭露三種設定，每個組合跑 10 次 | Full、Concat | Sharded 平均掉 39%，Concat 是 Full 的 95.1%；A 平均 −16%、U 平均 +112% | 日誌分析：過早作答、答案越改越長而不撤回舊假設、中間輪次被忽略、回應冗長 | [arXiv:2505.06120] |
| IC-DST Figure 5（本章計入） | 每一輪的 JGA | 狀態變化對累積狀態；沒有 gold 上一輪狀態的對照 | 狀態變化維持高檔，累積與直接預測的完整狀態都隨輪次下降（只有圖） | 後段輪次的狀態 slot 較多 | [arXiv:2203.08568] |
| MT-Bench-101（T7 計入） | 同一個模型分別吃金上下文與自己預測的上下文 | 金上下文 | 用自己的上下文時分數隨輪數下降；只用 ChatGLM3-6B、SI 與 IC 兩個任務，只有 Figure 6 的曲線 | 錯誤累積與傳播 | [arXiv:2402.14762] |
| CollabLLM（T1 計入） | 真人寫作研究的逐輪評分 | CollabLLM、Proactive Base | Base 在第 6–9 輪評分下滑，另兩者隨輪數上升（只有圖） | 較長的對話裡使用者體驗變差 | [arXiv:2502.00640] |
| 📖 2304.04256（T1） | ChatGPT 在同一個 session 裡逐輪更新狀態 | 沒有 | 約 10 輪之後可能忘記第一輪給的 schema 與規則，沒有量化比例 | context 長度限制 | [arXiv:2304.04256] |
| 📖 Drift No More | 逐輪 KL 散度 | 插入目標提醒 | 宣稱漂移有界、提醒後 KL 下降 | 漂移收斂到均衡 | [arXiv:2510.07777] |

**被提出的機制至少有四種，彼此沒有交叉檢驗（撰寫本章時的比較，範圍限本調研讀過的論文）。**

- 模型自己的作答行為：過早作答、不撤回先前的假設，出自 Lost in Conversation 的日誌分析 [arXiv:2505.06120]。
- 任務本身在後段變難：後段輪次的狀態 slot 較多，是 IC-DST 對 Figure 5 的解釋，全文沒有另做實驗驗證 [arXiv:2203.08568]。
- 錯誤沿上下文累積：MT-Bench-101 的金上下文對照佐證這一條，但只有一個模型、兩個任務、只有圖 [arXiv:2402.14762]。
- 早先的指示被遺忘：2304.04256 歸因於 context 長度，只有質性描述；Drift No More 的提醒實驗也預設這一條，兩篇都是 📖 [arXiv:2304.04256][arXiv:2510.07777]。

這幾篇的設定只有局部重疊，沒有一篇把兩種以上的機制放在同一個設定裡分開。最接近的交會點是 MT-Bench-101 的 SI 任務：它本來就把任務要求與輸入分兩輪給，和 Lost in Conversation 的逐片揭露同形；但 MT-Bench-101 變的是上下文由誰產生，Lost in Conversation 變的是資訊怎麼切，兩邊沒有交叉 [arXiv:2402.14762][arXiv:2505.06120]。依各自筆記，Lost in Conversation 沒有把 assistant 的前幾輪回應換成正確版本的對照；MT-Bench-101 的主評估一律用金上下文，金上下文與自身上下文的對照只在 ChatGLM3-6B 的 SI、IC 兩個任務上做，以 Figure 6 的曲線呈現、沒有數值 [arXiv:2505.06120][arXiv:2402.14762]。所以「自己說錯的話」占退化多少，只有 MT-Bench-101 那一個模型、兩個任務的定性答案，Lost in Conversation 則沒有這種對照 [arXiv:2402.14762][arXiv:2505.06120]。

**指令保持。** 上表只有兩篇 📖 碰到「早先給的指令在後面是否仍被遵守」，計入的論文裡沒有一篇直接量（見〈這一章回答什麼〉）[arXiv:2304.04256][arXiv:2510.07777]。依標題直接研究這件事的論文，以及多輪指令遵循的基準，都沒有進任何節點的候選池；這一塊是本調研的收錄缺口，可以補的論文列在〈已知缺口與未讀〉的「指令保持與多輪退化的基準」。

## 方法比較

### 跨子領域：狀態怎麼跨輪維護

這張表把三個子領域的維護方式放在同一條軸上。「失敗型態從哪來」一欄逐列標明來源：除了 Lost in Conversation 那一列出自論文自己的日誌分析，其餘各列的失敗型態都是精讀時依論文或公開程式碼推得的。逐篇核對表中各篇的 limitations_stated，碰到這些失敗型態的只有 TripPy：它在未來工作裡自承更新策略簡單，想加入部分遺忘的選項，對應規則覆寫那一列 [arXiv:2005.02877]。相鄰但不相同的有兩處：SimpleTOD 自承多回合、長上下文的狀態追蹤只有定性範例，沒提截斷 [arXiv:2005.00796]；Hindsight 自承的是意見與信念層還不支援受控遺忘與時間感知的信念修正，不是表中 world／experience 事實那一項 [arXiv:2512.12818]。「有沒有量到」一欄記錄有沒有實驗真的量到這個失敗（撰寫本章時逐篇對照筆記彙整）。

| 維護方式 | 代表 | 已知的失敗型態 | 失敗型態從哪來 | 有沒有量到 | 出處 |
| --- | --- | --- | --- | --- | --- |
| 固定係數插值 | NBT | 照字面實作時，沒被重提的目標會衰減到門檻以下（精讀時推算） | 精讀時推算 | 沒有 | [arXiv:1606.03777] |
| 規則覆寫（非 none 即覆寫，none 沿用舊值） | GLAD、TripPy | 沒有刪除路徑，寫錯的值留到同一 slot 被再次指定 | 精讀時分析；TripPy 在未來工作中自承更新策略簡單，想加入部分遺忘的選項 | 沒有；TripPy 沒量化錯誤傳播占 JGA 損失多少 | [arXiv:1805.09655][arXiv:2005.02877] |
| 隱式遞迴（RNN 隱藏狀態） | SUMBT | 狀態不能檢查，也不能從外部修正後再接下去 | 精讀時分析 | 沒有；沒有按回合數分析錯誤 | [arXiv:1907.07421] |
| 歷史視窗內每輪重新生成 | TRADE | 視窗外提過的值只能靠當輪重新找到 | 精讀時分析 | 沒有 | [arXiv:1905.08743] |
| 完整歷史每輪重算 | SimpleTOD、TOD-BERT、T5DST、TransferQA、DaP、D3ST | 歷史超過輸入上限時會被截斷；SimpleTOD 在 1,024 token 截斷，DaP 的公開程式碼右側截斷會截掉 slot 提示（精讀時依程式碼推得） | 精讀時分析；DaP 那一項依公開程式碼；SimpleTOD 自承長上下文的狀態追蹤只有定性範例，沒提截斷 | 沒有；沒有一篇按對話長度分析 JGA | [arXiv:2005.00796][arXiv:2004.06871][arXiv:2105.04222][arXiv:2109.04655][arXiv:2109.07506][arXiv:2201.08904] |
| 差分累加 | SGD 原型、IC-DST | 判錯的差分會沿累積狀態一路帶下去 | 精讀時分析 | 沒有；IC-DST 只有 Figure 5 的逐輪曲線，沒做 gold 與預測上一輪狀態的對照 | [arXiv:1909.05855][arXiv:2203.08568] |
| 程式合併、只增不刪 | FnCTOD | 無法撤回先前的條件，每輪只更新一個領域（精讀時依公開程式碼） | 精讀時依公開程式碼 | 沒有 | [arXiv:2402.10466] |
| 只增不刪的記憶流＋反思寫回 | Generative Agents | 過時事實不會失效，未驗證的反思寫回後會被再引用 | 精讀時分析 | 沒有 | [arXiv:2304.03442] |
| 追加＋遺忘衰減 | MemoryBank | 使用者改口時新舊事實同時被取回；程式碼的遺忘公式方向與論文相反 | 精讀時分析；程式碼那一項依筆記轉述，算術另經程式驗證 | 沒有；遺忘沒有任何量化結果 | [arXiv:2305.10250] |
| agent 以函式呼叫自己改寫 | MemGPT | 覆寫是否正確無從得知 | 精讀時分析 | 只有一則質性範例 | [arXiv:2310.08560] |
| Add／Merge | RMM | 沒有刪除或覆寫，新舊矛盾只能靠合併句的改寫表達 | 精讀時分析 | 沒有；LongMemEval 只報總準確率 | [arXiv:2503.08026] |
| ADD／UPDATE／DELETE／NOOP | Mem0（圖版 Mem0g 改為軟失效） | 硬刪讓「以前是什麼」答不了；錯刪的長期影響不明 | 精讀時分析 | 沒有；四種操作都沒被單獨量過 | [arXiv:2504.19413] |
| 事實寫入時不作廢、多半讀取時裁決 | Hindsight | world／experience 事實沒有寫入時的作廢機制，衝突主要留給時間中繼資料與答題 LLM；observation 重新產生時取最新或證據最多，opinion 與背景另有寫入時的更新規則（精讀時推測 knowledge-update 的增益來自讀取端） | 精讀時分析；論文另自承意見與信念層還不支援受控遺忘，不是這一項 | 有 knowledge-update 分項，但沿用 LongMemEval 官方評審提示詞，該提示詞在最後答案是新值時接受新舊資訊並列 | [arXiv:2512.12818][arXiv:2410.10813] |
| 沒有顯式狀態（原始對話歷史） | Lost in Conversation 的被測 assistant | 過早作答、不撤回舊假設、中間輪次被忽略 | 論文自己的日誌分析 | 有：Sharded 平均掉 39% | [arXiv:2505.06120] |

這張表可能少了一類：微調路線上以上一輪狀態為顯式輸入的 DST。TripPy 的原文把 SOM-DST 描述成上一輪狀態加上一輪對話一起送進 BERT [arXiv:2005.02877]；SOM-DST 本身本調研沒讀，它怎麼更新、有什麼失敗型態都無法填進這張表（見〈已知缺口與未讀〉的「傳統 DST 裡以上一輪狀態為輸入的路線」）。
...
| 📖 Context equilibria（Drift No More） | 逐輪 KL(測試模型 ‖ 參考模型) 當漂移；擬合 ΔD_t = a + b·D_t 取均衡；固定輪次插入目標提醒 | 需要參考模型的 token 分佈；不訓練 | 自建 8 輪受限改寫任務；τ-bench 的使用者模擬器 | 提醒後 KL −7.47%／−6.45%／−11.81%；Table 6 六列的 b 從 −2.444 到 −0.957 | [arXiv:2510.07777] |
...
- 兩個 DST 子領域用的是 DSTC2（口語，測試用 ASR 假設）、WOZ 2.0、MultiWOZ 的 2.0 到 2.4 各版、SGD 與它的改寫版 SGD-X，以及 M2M [arXiv:1606.03777][arXiv:1805.09655][arXiv:1905.08743][arXiv:1909.05855][arXiv:2201.08904][arXiv:2109.07506]。其中 MultiWOZ 2.1、2.4 與 SGD-X 的原始論文本調研沒讀，只能從用它們的論文反推（見〈已知缺口與未讀〉）[arXiv:2201.08904][arXiv:2203.08568]。
...
第三個子領域的兩個主要考場，原始論文在 T7 計入，本章據此補上它們自己的設計：

- LoCoMo：7,512 題分五類（single-hop、multi-hop、temporal、open-domain、adversarial），每題標了含答案的證據 turn ID [arXiv:2402.17753]。精讀時發現，公開版只保留 50 段中最長的 10 段，論文的表在公開資料上無法重現 [arXiv:2402.17753]。精讀時發現，它的事件圖只會單調累積，沒有事實被更新或推翻的題目 [arXiv:2402.17753]。
- LongMemEval：500 題，七種題型對應五種能力，其中一種是知識更新（KU）；標準設定 S 每題約 115k token、M 約 1.5M token，另有只給證據 session 的 oracle [arXiv:2410.10813]。GPT-4o judge 依題型換提示詞，KU 題的回答同時出現舊資訊、但最後答案是更新後的值，仍判對 [arXiv:2410.10813]。精讀時發現，歷史裡的助理回覆全是事先生成的，受測系統自己的回覆不影響後續輪次，所以「系統在對話中記錯並帶下去」測不到 [arXiv:2410.10813]。
- DMR：MemGPT 以 MSC 延伸出第 6 個 session 組成 [arXiv:2310.08560]。MSC 的原始論文不在本調研的筆記裡。

**主指標。** 兩個 DST 子領域的主指標都是 joint goal accuracy（JGA）：某一輪的整份狀態全對才算對，沒有值的 slot 要預測 none [arXiv:1905.08743][arXiv:2005.02877][arXiv:2007.12720]。第三個子領域沒有量「更新做得對不對」的指標，主指標是下游問答的 accuracy、LLM judge 分數、可信度排名與任務分數 [arXiv:2304.03442][arXiv:2310.08560][arXiv:2504.19413][arXiv:2505.06120]。中間層的指標倒是有，量的是有沒有取回證據：RMM 在 LongMemEval 報 Recall@5，LongMemEval 原論文以人工標註的證據位置算 Recall@k 與 NDCG@k，LoCoMo 原論文以證據 turn ID 算 RAG 的 recall@k [arXiv:2503.08026][arXiv:2410.10813][arXiv:2402.17753]。取回了證據不等於狀態被正確更新，但它至少把「沒找到」和「找到了卻答錯」分開（撰寫本章時的判斷）。
...
- TRADE 在 2.0 的 test JGA 是 48.62 [arXiv:1905.08743]。IC-DST 的 Table 1 在 TRADE 那一列列了全量資料下 2.1 的 46.00 與 2.4 的 55.05，沒有交代是重跑還是引用；2.1 那一格和 MultiWOZ 2.2 論文表中 TRADE 在 2.1 的 0.460 一致 [arXiv:2203.08568][arXiv:2007.12720]。2.0 到 2.4 差 55.05 − 48.62 = 6.43 個百分點，但兩個數字出自不同論文的表，設定是否相同無從確認（撰寫本章時比對全文）[arXiv:1905.08743][arXiv:2203.08568]。
- 同一篇論文、同一個模型在不同版本上的對照，本章計入的論文裡有三處（撰寫本章時依筆記與全文清點）：
  - IC-DST 自己的 few-shot 結果（2.1 對 2.4）：1%／5%／10% 資料時，2.1 是 43.13／47.08／48.67，2.4 是 48.35／55.43／56.88，依序差 48.35 − 43.13 = 5.22、55.43 − 47.08 = 8.35、56.88 − 48.67 = 8.21 個百分點 [arXiv:2203.08568]。
  - D3ST 的同一組模型在 2.1／2.2／2.3／2.4 都報了 JGA：Base 是 54.2／56.1／59.1／72.1，XXL 是 57.8／58.7／60.8／75.9 [arXiv:2201.08904]。但 D3ST 只對 2.1 套用 TRADE 的前處理腳本，2.2 到 2.4 都不做前處理，所以 2.1 與後三版之間的差距混著版本與前處理兩個因素 [arXiv:2201.08904]。
  - MultiWOZ 2.2 的 Table 4（2.0 到 2.2）：TRADE 在 2.0／2.1／2.2 是 0.486／0.460／0.454，DS-DST 是 0.522／0.512／0.517，SGD-baseline 在 2.1／2.2 是 0.434／0.420 [arXiv:2007.12720]。

  IC-DST 與 D3ST 用到的 2.1 與 2.4 各自修了哪些標註，要看兩版的原始論文，本調研都沒讀（見〈已知缺口與未讀〉）[arXiv:2203.08568][arXiv:2201.08904]。
...
- **狀態用自己的預測，但沒量傳遞效應。** TripPy、SGD 原型、IC-DST 測試時都在自己的預測上累積，但沒有一篇做「餵 gold 的上一輪狀態」與「餵預測的上一輪狀態」的對照 [arXiv:2005.02877][arXiv:1909.05855][arXiv:2203.08568]。最接近的是 IC-DST 的 Figure 5：累積狀態的 JGA 隨輪次下降，但論文歸因於後段 slot 較多，也沒有 gold 對照可以把錯誤傳遞分出來（見〈LLM 時代的 zero/few-shot DST〉）[arXiv:2203.08568]。
- **整段對話都由被測模型自己接下去。** 本章計入的論文裡只有 Lost in Conversation 這樣量，而它量到的是沒有顯式狀態時的退化：Sharded 平均掉 39% [arXiv:2505.06120]。T7 計入的 MT-Bench-101 另外做了金上下文與模型自己預測的上下文的對照，用自己的上下文時分數隨輪數下降；但只用 ChatGLM3-6B、SI 與 IC 兩個任務，只有 Figure 6 的曲線 [arXiv:2402.14762]。
...
- **ConvLab-2 上 JGA 與任務成功率名次相反（精讀 SUMBT 時從 ConvLab-2 的 GitHub README 查到，https://github.com/thu-coai/ConvLab-2）。** 依 SUMBT 筆記的轉述，README 用自己的評估腳本得到 SUMBT 的 JGA 0.30、TRADE 0.40，但接上規則式政策與使用者模擬器後，SUMBT 的 success rate 29.4 反而高於 TRADE 的 20.1 [arXiv:1907.07421]。這組數字的出處是工具包的 README，不是 SUMBT 論文，也不是 ConvLab-2 的論文（本調研沒讀，見〈已知缺口與未讀〉的「工具包層級的端對端評估」）；README 會隨版本更新，本調研沒有對過論文版本。
...
- Mem0 沒量抽取的精確率與召回率，也沒量 UPDATE／DELETE 判斷的正確率（精讀時發現）[arXiv:2504.19413]。而它的主基準 LoCoMo 本身就沒有事實被更新或推翻的題目（精讀 LoCoMo 時發現），所以即使要量，這個基準也量不到 [arXiv:2402.17753]。
- RMM 在 LongMemEval 只報總準確率，不報知識更新等分題型結果（精讀時發現）[arXiv:2503.08026]。
- Hindsight 有報 knowledge-update 分項，同一個 OSS-20B 從 full-context 的 60.3 升到 84.6 [arXiv:2512.12818]。但這一題型的評審提示詞規定，最後給出更新後的答案、同時提到舊資訊也算對；這條規定不是 Hindsight 自己訂的，而是 LongMemEval 原論文的題型專屬提示詞，Hindsight 的附錄照搬 [arXiv:2410.10813][arXiv:2512.12818]。所以凡是照官方提示詞評 KU 的系統都受這一條影響，這個分數也不能證明狀態層真的做了更新（精讀時發現）[arXiv:2410.10813][arXiv:2512.12818]。RMM 在 LongMemEval 用的是自己寫的通用評審提示詞（附錄 D.3），沒有這一條，但它也不報 KU 分項 [arXiv:2503.08026]。
- LongMemEval 原論文在自己的最佳記憶設計下檢查錯誤：約 90% 的答對案例伴隨正確檢索，其餘約 10% 多半是只取回更新後資訊的 knowledge-update 題 [arXiv:2410.10813]。也就是說，KU 題可以不取回舊值就答對；這和「狀態有沒有被正確更新」仍是兩件事（撰寫本章時的判斷）。
...
- RMM 的 Gemini-1.5-Pro 同時是評審與被評的生成器之一，提示詞是自己的通用版 [arXiv:2503.08026]。
...
兩份全文都沒寫明這組題數，它是反推出來的 [arXiv:2504.19413][arXiv:2512.12818]。所以兩篇的 LoCoMo 總分都有一半以上的權重落在標為 Open Domain 的那一欄 [arXiv:2504.19413][arXiv:2512.12818]。

**但這一欄裝的很可能不是 LoCoMo 原始的 open-domain 題（程式驗證，照欄名的解讀被推翻）。** Hindsight 的欄名沿用 Backboard 與 Mem0，Mem0 論文也沒列題數，所以只看這兩份全文無法判定 [arXiv:2504.19413][arXiv:2512.12818]。T7 計入的 LoCoMo 原論文補上了缺的那一塊：全集 50 段對話共 7,512 題，single-hop 2,705、multi-hop 1,104、temporal 1,547、open-domain 285、adversarial 1,871 [arXiv:2402.17753]。論文本身沒講公開版的範圍；精讀時依 LoCoMo 的 GitHub README 查到，公開版只保留 50 段中最長的 10 段 [arXiv:2402.17753]。若公開版的題目是全集的子集、類別沒被改標，任何一欄的題數都不能超過全集同類的題數；標為 Open Domain 的欄有 841 題，全集的 open-domain 卻只有 285 題，照欄名對應不可能成立 [arXiv:2402.17753][arXiv:2512.12818]。排除 adversarial 後全集剩 7,512 − 1,871 = 5,641 題，single-hop 占 2,705 ÷ (7,512 − 1,871) ≈ 48.0%，open-domain 只占 285 ÷ (7,512 − 1,871) ≈ 5.1% [arXiv:2402.17753]。程式列舉全部 4! 種「欄名 → 原始類別」的對應，比例最接近全集的是：Open Domain 欄＝原始 single-hop、Multi-Hop 欄＝原始 open-domain；這個對應下四欄各占全集同類的比例，從 321 ÷ 1,547 ≈ 20.7% 到 96 ÷ 285 ≈ 33.7%，接近整體的 1,540 ÷ (7,512 − 1,871) ≈ 27.3% [arXiv:2402.17753][arXiv:2512.12818]。Single-Hop 與 Temporal 兩欄的歸屬，比例配對分不清（見〈程式驗證〉）。所以兩篇的 LoCoMo 總分最可能是由原始的 single-hop 題主導，不是 open-domain 題 [arXiv:2402.17753][arXiv:2504.19413][arXiv:2512.12818]。這是強旁證，不是證明：公開版是挑最長的 10 段，不是隨機抽樣，而「子集、沒改標」這個前提仍要 LoCoMo 資料集的 category 欄位才能確認。
...
本章有七條主張送程式驗證，拆成十六條子主張：前四條是第一輪送驗的候選，後三條是依修訂時的缺口清單補驗的。每條主張一支程式，只用 Python 標準函式庫，輸入數字只取自精讀筆記與快取全文。完整輸出、輸入出處與逐條判讀見 [../verify/02-results.md](../verify/02-results.md)。「來源」一欄標的是主張最初從哪裡來：精讀時發現的是筆記 limitations_observed 裡精讀 agent 的分析，撰寫時推算或核對的是撰寫本章時自己的推算與比對。
...
| LoCoMo 的總分由 Open Domain 主導 | 精讀時發現 | **由下兩列取代**：第一輪只用兩份全文，判為無法判定；補驗時接上 T7 的 LoCoMo 原論文筆記，照欄名的解讀被推翻 | 〈記憶系統的評估〉 | [02-locomo-weights.py](../verify/02-locomo-weights.py) | [arXiv:2512.12818][arXiv:2504.19413] |
| 標為 Open Domain 的欄就是 LoCoMo 原始的 open-domain 類別 | 撰寫時推算（修訂時補驗） | **照欄名否定**：那一欄 841 題，全集 50 段的 open-domain 只有 285 題，子集的題數不可能超過全集。前提是公開版題目是全集的子集、類別沒被改標，這個前提快取無法驗證 | 〈記憶系統的評估〉 | [02-locomo-category-map.py](../verify/02-locomo-category-map.py) | [arXiv:2402.17753][arXiv:2512.12818] |
| 那一欄最可能是哪一類 | 撰寫時推算（修訂時補驗） | **強旁證，不是證明**：全部 4! 種對應中，比例最接近全集的是 Open Domain 欄＝原始 single-hop、Multi-Hop 欄＝原始 open-domain；Single-Hop 與 Temporal 兩欄的歸屬分不清。公開版是挑最長的 10 段，不是隨機抽樣，比例距離只能當描述 | 同上 | [02-locomo-category-map.py](../verify/02-locomo-category-map.py) | [arXiv:2402.17753][arXiv:2512.12818] |
| Drift No More 的恢復力迴歸在 i.i.d. 雜訊下必然得到 b≈−1、R²≈0.5，「斜率為負」分不出恢復力 | 精讀時發現 | **證實**：解析推導與模擬一致，而且在測過的各種段數下 b 幾乎一定是負的 | 〈跨子領域的爭議〉 | [02-drift-iid-null.py](../verify/02-drift-iid-null.py) | [arXiv:2510.07777] |
| Table 6 的 b 與 R² 就是虛無模型的期望值 | 精讀時發現 | **只對兩列成立**：GPT-4.1 與 70B 的 Baseline 列落在虛無模型中心，在測過的段數（1、5、20、100）下都在 95% 區間內；另外四列不在中心，方向也不是有記憶的恢復，8B Reminders 的 −2.444 連平穩的一階自迴歸都給不出來。論文沒報段數；在測過的 20 段與 100 段下這四列都在區間外，段數少時其中幾列仍在區間內 | 同上 | [02-drift-iid-null.py](../verify/02-drift-iid-null.py) | [arXiv:2510.07777] |
| MemoryBank 程式碼的 exp(-t/5*S) 依運算順序等於 e^(−t·S/5)，召回越多次忘得越快，與論文公式方向相反 | 精讀時發現 | **證實**（算術）：語法樹確認是左結合，隨 S 嚴格遞減；筆記算出的三個數字都重現 | 〈LLM agent 的多輪狀態維護、對話記憶與多輪退化〉 | [02-memorybank-forgetting.py](../verify/02-memorybank-forgetting.py) | [arXiv:2305.10250] |
| 公開程式碼確實寫成那一行 | 精讀時發現 | **無法判定**：快取只有論文全文，這一行只來自筆記的轉述 | 同上 | [02-memorybank-forgetting.py](../verify/02-memorybank-forgetting.py) | [arXiv:2305.10250] |

**被擋住的驗證。** MultiWOZ 上目標變更、撤回與跨領域同輪修改的實際比例，原本可以從 2.1 或 2.2 的逐輪狀態直接統計：slot 被移除、值被改寫、同一輪跨領域變更的輪次各占多少。這個數字是「JGA 看不出刪除類錯誤」「資料的更新型態太窄」的核心證據。但快取裡沒有 MultiWOZ 的資料檔，本調研的規則也不允許下載，所以沒有驗；正文相關的句子都改寫成「本調研讀過的論文沒有統計，本調研也統計不了」[arXiv:1810.00278][arXiv:2402.10466]。

**沒有送驗的推算。** 本章另有幾處精讀時或撰寫時的推算沒有送驗：SGD Alarm 領域空狀態輪次比例的下界 [arXiv:1909.05855]、Generative Agents 的 Cohen's d 重算 [arXiv:2304.03442]、D3ST SS(JGA) 隨正確率機械下降的推論 [arXiv:2201.08904]、Lost in Conversation 中 Snowball 與 Recap 補回比例的重算 [arXiv:2505.06120]，以及 MultiWOZ 回合數的重算 [arXiv:1810.00278]。其中寫成四則算式的部分，數字比對工具已驗算過算術；但推算所依賴的前提沒有經過程式驗證，例如 SGD 每個服務的 slot 上限與部分分數的合併方式、SS(JGA) 的確切定義。所以正文維持原來「推算、未經程式驗證」的標註。
...
撰寫本章時的判斷：三組跨 session 的條件，在歷史長度（約 26K 對約 115K token）、backbone 與評審上都不同，這三篇都沒有做對話長度的掃描 [arXiv:2504.19413][arXiv:2512.12818][arXiv:2503.08026]。精讀時發現，Mem0 說全文脈絡的成本會急遽成長，全文卻沒有任何長度掃描實驗 [arXiv:2504.19413]。

T7 計入的兩篇基準原論文補上了一部分答案。

LongMemEval 全程用同一個 GPT-4o judge，有三個長度點：只給證據 session 的 oracle、每題約 115k token 的 S、約 1.5M token 的 M [arXiv:2410.10813]。

- 長脈絡模型直接讀完 S 時，GPT-4o 從 oracle 的 0.870 掉到 0.606，Llama 3.1 70B 從 0.744 掉到 0.334 [arXiv:2410.10813]。
- 附錄 Table 8 替另外五個模型報了三個點。S 上直接讀全文與記憶檢索都有量，五個模型都是記憶檢索較高，例如 Mistral-Nemo-Instruct-2407 直接讀 0.162、以 round 為單位的記憶檢索 0.640；M 只量了記憶檢索，同一模型是 0.554（撰寫本章時核對全文）[arXiv:2410.10813]。
- 它也拆開比較了記憶的索引設計：把 session 拆成 round 對強閱讀模型有利，壓成摘要或事實整體會變差，唯獨跨 session 推理題受惠於事實分解 [arXiv:2410.10813]。精讀時提醒，這組比較的抽取器一律是 Llama 3.1 8B，「壓縮有害」可能部分反映抽取器的品質，而不是壓縮本身 [arXiv:2410.10813]。

LoCoMo 原論文用同一個 gpt-3.5-turbo-16k、同一把 token F1，做了一組長度掃描與一組壓縮對照 [arXiv:2402.17753]：

- 長度掃描：分別給 4K、8K、12K、16K token 的對話，總體 F1 依序是 24.1、25.2、33.5、37.8 [arXiv:2402.17753]。
- 壓縮對原文：以同一個模型當 reader 做 RAG，以 observation 為檢索單位的 top-5 是 41.4，以原始對話輪為單位的 top-5 是 31.7，以 session 摘要為單位的 top-10 是 31.5 [arXiv:2402.17753]。

這組結果要附三條但書 [arXiv:2402.17753]：

- 尺是 token F1，不是 LLM 評審 [arXiv:2402.17753]。
- 對話平均只有約 9K token，16K 幾乎等於給全文（精讀時指出），所以這裡掃的是給多少上下文，不是對話長度 [arXiv:2402.17753]。
- observation 是生成對話時的副產品，附著證據 turn ID；精讀時指出，實際部署時要由受測系統自己抽取，所以「observation 最好」是在有特權資訊的條件下得出的 [arXiv:2402.17753]。精讀時另外指出，observation 是 gpt-3.5-turbo 寫的斷言句，措辭可能剛好對上 token F1 [arXiv:2402.17753]。

所以在本調研讀過的論文裡，「壓成狀態」與「保留原文」誰好只有部分答案，而且兩個基準的方向不同（撰寫本章時的比較）：LongMemEval 的索引比較裡，以原文的 round 為單位較好，壓成摘要或事實整體變差，跨 session 推理題例外；LoCoMo 的比較裡，壓成 observation 的 41.4 勝過直接讀全文的 16K 37.8，也勝過以原始對話輪檢索的 31.7，session 摘要的 31.5 則輸給全文 [arXiv:2410.10813][arXiv:2402.17753]。至於長度，LongMemEval 在 GPT-4o judge 下只在 S 這一個長度同時量了直接讀全文與記憶檢索；它另在 3 到 6 個 session 的短歷史上比過直接讀全文與商用助理的記憶，但那組由人工判分，記憶是商用產品（精讀時指出）[arXiv:2410.10813]。LoCoMo 只在全長同時量了兩者 [arXiv:2402.17753]。本調研讀過的論文裡，沒有一篇在同一模型、同一 LLM 評審下，於兩個以上的對話長度同時量全文脈絡與外部記憶，所以兩者在哪個長度交叉仍然不知道 [arXiv:2203.08568][arXiv:2505.06120][arXiv:2504.19413][arXiv:2512.12818][arXiv:2410.10813][arXiv:2402.17753]。
...
- **反證：迴歸診斷沒有鑑別力（精讀時指出，程式驗證證實）。** 若 D_t 只是彼此獨立的雜訊，把 ΔD_t = D_{t+1} − D_t 對 D_t 做 OLS，b 的母體值是 −1、R² 是 0.5，而且 b 幾乎一定是負的；所以「斜率全為負」分不出有恢復力還是根本沒有動態 [arXiv:2510.07777]。Table 6 共六列（GPT-4.1、LLaMA-3.1-70B、LLaMA-3.1-8B 各有 Baseline 與 Reminders），b 依序是 −0.957、−1.250、−1.049、−1.007、−1.432、−2.444，R² 依序是 0.494、0.626、0.494、0.278、0.723、0.538 [arXiv:2510.07777]。把六列放進虛無模型的模擬分佈（每段 8 輪）之後，GPT-4.1 與 70B 的兩個 Baseline 列正落在虛無模型的中心，在測過的 K = 1、5、20、100 段下都落在 95% 區間內；區間隨段數變窄，段數再多時這兩列也可能落到區間外 [arXiv:2510.07777]。另外四列都不在中心，而且偏離的方向不是「有記憶的恢復」（那會讓 b 落在 −1 與 0 之間）：GPT-4.1 Reminders 與 8B 兩列的 b 小於 −1，其中 8B Reminders 的 −2.444 連平穩的一階自迴歸都給不出來，也是唯一在測過的每一種段數下都落在區間外的一列；70B Reminders 的 R² 0.278 則偏低（見〈程式驗證〉）[arXiv:2510.07777]。論文沒報每個條件用了幾段對話；在測過的 20 段與 100 段下這四列都落在區間外，段數少時其中幾列仍在區間內 [arXiv:2510.07777]。
...
- 精讀時指出，Recap 與 Snowball 都只是把使用者原話重貼進已經含有錯誤嘗試的上下文；「由 LLM 把目前狀態彙整成結構化摘要，放進乾淨的上下文重問」沒被測 [arXiv:2505.06120]。而這正是 MemGPT 的 working context、RMM 的主題摘要、Mem0 的事實抽取在做的事；依各篇筆記，這三個系統沒有一篇在 session 內逐輪補齊的設定下被測過（撰寫第 4 組草稿時整理）[arXiv:2310.08560][arXiv:2503.08026][arXiv:2504.19413][arXiv:2505.06120]。本節點有一篇沒讀的候補，依標題研究以遞迴摘要維護長對話記憶，離這個組合最近（見〈已知缺口與未讀〉的「本節點的其餘候補」）；它沒讀，所以「沒被測」只對本調研讀過的論文成立。
...
- ConvLab-2 的 GitHub README（精讀時查到，https://github.com/thu-coai/ConvLab-2）：換成它的前處理與評估程式，是 0.30 [arXiv:1907.07421]。出處是工具包的 README，不是 ConvLab-2 的論文，後者本調研沒讀。
...
- **Hindsight 表中的 Zep（精讀時發現，程式驗證證實並修正說法）。** 總分寫 75.14，但照原表欄位加權只得 (74.11×282 + 66.04×96 + 67.71×841 + 79.79×321) ÷ 1,540 ≈ 71.30，對不起來 [arXiv:2512.12818]。程式驗證另外發現，把 Zep 列後三欄輪換（66.04 移到 Temporal、67.71 移到 Multi-Hop、79.79 移到 Open Domain），加權就是 (74.11×282 + 67.71×96 + 79.79×841 + 66.04×321) ÷ 1,540 ≈ 75.13，四格也都能還原成整數題數；在 4 × 3 × 2 = 24 種欄位排列中，只有這一種如此 [arXiv:2512.12818]。所以比較精確的說法是：Zep 列的類別對應與其他列不一致。這是抄錯，還是來源用了不同的類別排序，快取分不出來，因為 Zep 部落格的原表不在快取裡 [arXiv:2512.12818]。LoCoMo 原論文的題數讓「欄名會錯位」不只是理論上的疑慮：〈記憶系統的評估〉已說明，其他各列標為 Open Domain 的欄很可能裝的是原始的 single-hop 題 [arXiv:2402.17753][arXiv:2512.12818]。
...
- 另一種做法本調研只間接讀到。Hindsight 的相關研究段落把 Zep 描述為雙時間的時間知識圖，分別追蹤事實何時有效、何時被記錄 [arXiv:2512.12818]。Zep 自己的論文本調研沒讀，它怎麼處理過時事實、有沒有量過這一步，本章無法判斷；它能檢驗的是本節「六篇的做法」是否涵蓋了主要的更新語意（見〈已知缺口與未讀〉的「記憶的時間知識圖」）。
...
- **本章計入的格式裡，更多的格式根本沒有修正的入口。** GLAD 與 TripPy 只覆寫不刪除，FnCTOD 只增不刪，Generative Agents 只增不刪 [arXiv:1805.09655][arXiv:2005.02877][arXiv:2402.10466][arXiv:2304.03442]。這一句只在本調研計入的格式裡成立；傳統 DST 裡有一篇本調研沒讀的論文能檢驗它（見〈已知缺口與未讀〉的「傳統 DST 裡以上一輪狀態為輸入的路線」）。
...
所以這條邊目前只做了一半：T5 那一側會產生修正，T2 這一側有能寫入的格式。表中判準 (a)、(b) 都不是「否」的論文裡，報了寫回之後下游數字的只有 WebCanvas 那一筆單次、未經檢定的下降，以及 LATS 拿掉反思的消融（HotPotQA EM 從 0.63 降到 0.58，精讀時指出只差 100 題中的 5 題）；兩者寫回的對象都不是對話狀態 [arXiv:2406.12373][arXiv:2310.04406]。計數的依據要講清楚，因為別處的計數可能不同：本章只數 (a)、(b) 都不是「否」、而且報了寫回後下游數字的論文。MemGPT 與 SWE-agent 的 (a)(b) 都是「是」，但兩篇都沒有報這一步的數字，所以不計入 [arXiv:2310.08560][arXiv:2405.15793]。Reflexion 有數字，反思相對只放上一次軌跡的 EPM 版多出 8%（CoT(GT)、100 題、只從圖讀出），但它的寫回發生在環境重置之後的下一次嘗試，判準 (b) 為「否」，所以也不計入 [arXiv:2303.11366]。表中其他有數字的論文不符合這兩個判準，例如 Generative Agents 的反思寫回不是修正，Reflexion 與 RMM 的寫回不在同一段互動內的狀態上 [arXiv:2304.03442][arXiv:2303.11366][arXiv:2503.08026]。沒有一篇量過「修正寫回之後，狀態本身有沒有變得更對」（撰寫本章時的判斷）[arXiv:2310.08560][arXiv:2406.12373][arXiv:2504.19413]。

## 未解問題

每一題最後標明卡住的性質。「領域沒解」指本調研讀過的論文裡沒有答案，也沒有已知的未讀論文專門處理；「本調研沒收」指有已知的論文可能回答，但本調研沒讀，對應〈已知缺口與未讀〉的條目。兩者都只依本調研讀過的範圍判斷。

1. **更新這一步本身怎麼量。** 三個子領域在〈方法比較〉的跨子領域表裡列了十三種維護方式，但量的一直是下游結果：JGA、問答準確率或可信度 [arXiv:1805.09655][arXiv:2005.02877][arXiv:2504.19413][arXiv:2512.12818]。卡在四處：
   - JGA 分不出「本輪抽錯」與「前面錯了一路帶下來」[arXiv:1805.09655]。
   - 記憶系統沒有操作層級的標註：本調研讀過的論文沒有一篇為「該抽出哪些事實、該 UPDATE 還是 DELETE」標過 ground truth [arXiv:2310.08560][arXiv:2503.08026][arXiv:2504.19413][arXiv:2512.12818]。
   - 最接近的 knowledge-update 題型，評審接受新舊答案並列；這條規定出自 LongMemEval 官方的題型專屬提示詞，Hindsight 照搬 [arXiv:2410.10813][arXiv:2512.12818]。
   - Mem0 的主基準 LoCoMo 沒有事實被更新或推翻的題目（精讀 LoCoMo 時發現）[arXiv:2402.17753]。

   資料上的更新型態也窄：MultiWOZ 2.0 的蒐集設計明文只有一種目標變更，也就是以初始約束查無資料、鼓勵改用替代值（精讀時據此分析）[arXiv:1810.00278]。實際資料裡撤銷、更正或回指修改有多少，本調研讀過的論文都沒有統計，本調研自己也統計不了（見〈程式驗證〉）[arXiv:1810.00278]。

   **性質：** 操作層級的標註與 KU 判準是領域沒解；資料的更新型態那一半，另有本調研沒讀的 MultiWOZ 修正版本可能相關（本調研沒收）。

2. **累積狀態的錯誤傳遞與每輪重算的長度效應，哪一個代價較大。** 累積派（TripPy、SGD 原型、IC-DST、FnCTOD）沒有做 gold 與預測上一輪狀態的對照；最接近的 IC-DST Figure 5 只畫了逐輪曲線，論文把累積狀態的下降歸因於後段 slot 較多 [arXiv:2005.02877][arXiv:2203.08568][arXiv:2402.10466]。重算派（SimpleTOD 與第二個子領域的五篇微調）沒有一篇按對話長度分析 JGA [arXiv:2005.00796][arXiv:2201.08904]。卡在兩處：
   - 評估資料太短：IC-DST 所用的 MultiWOZ 平均每段只有 13.46 輪，SGD train 平均 20.44 turns [arXiv:2203.08568][arXiv:1909.05855]。
   - 本章計入的論文裡，讓模型整段在自己的歷史上接下去的只有 Lost in Conversation，被測的 assistant 又沒有任何顯式狀態 [arXiv:2505.06120]。T7 計入的 MT-Bench-101 做了金上下文與自身上下文的對照，但只有一個模型、兩個任務、只有圖 [arXiv:2402.14762]。

   **性質：** 在本調研讀過的論文裡是領域沒解；以上一輪狀態為輸入的 SOM-DST 沒讀，它能檢驗「累積派都沒做 gold 對照」這一句（本調研沒收）。

3. **一把可以跨論文比較的尺。** JGA 在 slot 集合、標籤正規化、資料版本、空狀態輪次算不算、種子數上各不相同，同一個 MDBT 可以從 80.9 變成 17.98，同一個 SUMBT 可以是 0.4240、0.488–0.491 或 0.30（0.30 出自 ConvLab-2 的 README）[arXiv:1810.00278][arXiv:1905.08743][arXiv:1907.07421]。「zero-shot」至少有五種意思，逐領域平均與多領域 JGA 可以差到 62.59 − 38.71 = 23.88 [arXiv:2402.10466]。記憶系統的 LLM 評審寬鬆度也各篇不同 [arXiv:2310.08560][arXiv:2504.19413][arXiv:2512.12818]。卡在：這些選擇都不寫進指標名稱；評估程式還可能因為相依版本而靜默改變語意（SUMBT，精讀時發現）[arXiv:1907.07421]。
...
5. **刪除、撤回與衝突的語意。** 以下是本章各篇的做法：
   ...
   - Zep 以雙時間的時間知識圖追蹤事實的有效期間；本調研只從 Hindsight 的相關研究段落讀到 [arXiv:2512.12818]。

   卡在基準：需要一個帶時間戳的事實變更序列，同時問「現在是什麼」與「以前是什麼」；Mem0 基本版的硬刪根本答不了後者（精讀時發現）[arXiv:2504.19413]。現有的兩個考場都不合用：LoCoMo 沒有更新題，LongMemEval 有 KU 題，但評審接受新舊並列 [arXiv:2402.17753][arXiv:2410.10813]。精讀 FnCTOD 時指出（未量化），MultiWOZ 上撤回與跨領域同輪修改都少見，JGA 因此看不出這類錯誤；這個比例本調研無法統計 [arXiv:2402.10466]。Hindsight 也自承意見與信念層還不支援受控遺忘與時間感知的信念修正 [arXiv:2512.12818]。

   **性質：** 基準那一半是領域沒解；更新語意的比較那一半，Zep 與 SOM-DST 沒讀（本調研沒收）。
...
6. **session 內的狀態維持，和跨 session 的記憶接不起來。**
   ...
   - 「多長以後外部記憶才划算」只有部分答案。LongMemEval 在同一個 GPT-4o judge 下有 oracle、約 115k token 的 S、約 1.5M token 的 M 三個點：長脈絡模型直接讀 S 明顯掉分，附錄 Table 8 的五個模型在 S 上都是記憶檢索勝過直接讀，M 只量了記憶檢索 [arXiv:2410.10813]。LoCoMo 原論文以同一個 gpt-3.5-turbo-16k、同一把 token F1 掃了 4K 到 16K 的上下文（總體 F1 24.1 到 37.8），但尺不是 LLM 評審，對話平均只有約 9K token，掃的是給多少上下文，不是對話長度 [arXiv:2402.17753]。約 26K token 時全文脈絡勝過 Mem0，約 115K token 時記憶系統大勝，但兩篇的模型與評審都不同 [arXiv:2504.19413][arXiv:2512.12818][arXiv:2503.08026]。本調研讀過的論文裡，沒有一篇在同一模型、同一 LLM 評審下，於兩個以上的對話長度同時量全文脈絡與外部記憶。「壓成狀態還是保留原文」的部分答案，在兩個基準上方向也不同（見〈完整歷史、上一輪狀態，還是外部記憶〉）[arXiv:2410.10813][arXiv:2402.17753]。
   - 範圍裡的「指令在多輪中的保持」，計入的論文沒有一篇直接量，只有兩篇 📖 碰到邊（見〈同一個 session 內的多輪退化與指令保持〉）[arXiv:2304.04256][arXiv:2510.07777]。

   卡在沒有實驗把兩邊放在一起：精讀時指出，「彙整後放進乾淨上下文重問」這個最直接的做法正好沒被測 [arXiv:2505.06120]。

   **性質：** 指令保持是本調研沒收，直接量它的論文與多輪指令遵循基準都沒進候選池；「彙整後重問」離本節點未讀的遞迴摘要候補最近（本調研沒收）；「多長以後」在部分答案之外是領域沒解。
...
7. **真正零曝光的 zero-shot，以及對 schema 措辭的穩健性。** 本章每一種 zero-shot 都帶著某種目標端曝光 [arXiv:2105.04222][arXiv:2201.08904][arXiv:2402.10466][arXiv:2004.06871]：
   ...
   換到 schema 慣例不同的資料集，D3ST XXL 的跨資料集 zero-shot 只有 28.9（SGD→MultiWOZ 2.4）與 23.1（MultiWOZ→SGD）[arXiv:2201.08904]。對措辭的穩健性只有 D3ST 在 SGD-X 上測過，XXL 最差的變體從 86.4 掉到 68.9 [arXiv:2201.08904]。唯一的量尺 SS(JGA) 本身也有疑問。撰寫第 3 組草稿時依 D3ST 的一句定義推論：若每輪 JGA 是 0／1、五個變體答對 k 次，母體標準差的變異係數是 √((5 − k)／k)，只由 k 決定，所以 SS 可能隨正確率機械地下降 [arXiv:2201.08904]。SGD-X 的原論文本調研沒讀，這個疑問在本章無從解決。

   **性質：** SS(JGA) 的定義與「描述有沒有用」是本調研沒收（SGD-X 與 Show, Don't Tell 沒讀）；零曝光的 zero-shot 在讀過的論文裡沒解，兩篇以提示做零樣本 DST 的候選也沒讀（本調研沒收）。

8. **JGA 能不能預測任務完成。** 本章有兩處反例：
   - ConvLab-2 的 GitHub README 上，SUMBT 的 JGA 低於 TRADE，端對端的 success rate 卻較高（精讀 SUMBT 時查到，https://github.com/thu-coai/ConvLab-2）[arXiv:1907.07421]。
   - SimpleTOD 自己也說，部分 oracle 設定下的名次推不到完整系統 [arXiv:2005.00796]。

   卡在：端對端分數同時受 DB 查詢、去詞彙化佔位符與政策模組影響，可以被設定灌高（精讀時推論）[arXiv:2005.00796][arXiv:2402.10466]。在同一條管線上同時量 JGA 與任務完成的比較，本調研讀過的材料只有 ConvLab-2 README 這一例，而且不是論文 [arXiv:1907.07421]。

   **性質：** 本調研沒收，ConvLab-2 的論文沒讀。

## 已知缺口與未讀

這一節列出本章知道、但沒有讀的論文與資料。每一條寫三件事：缺了哪一塊、哪些結論因此只在本調研讀過的論文裡成立、可以補的論文。未讀的論文只依標題說明它研究什麼，本章不引用它們的任何數字或結論。

- **傳統 DST 裡以上一輪狀態為輸入的路線。**
  - 缺了哪一塊：微調路線上把上一輪狀態當顯式輸入、再決定怎麼更新的 DST。本章只從 TripPy 的原文知道 SOM-DST 把上一輪狀態加上一輪對話一起送進 BERT [arXiv:2005.02877]。
  - 只在語料內成立的結論：跨子領域表的維護方式清單；「在本章計入的論文裡，IC-DST 的不同之處是把上一輪預測狀態的完整內容當顯式文字輸入」；Mem0 的操作集合以 IC-DST 為最近的前例；E2 一節「本章計入的格式裡，更多的格式沒有修正的入口」；未解問題 2 的「累積派都沒做 gold 對照」。
  - 可以補：Efficient Dialogue State Tracking by Selectively Overwriting Memory（arXiv 1911.03906，未讀）。TripPy 的參考文獻顯示這就是 SOM-DST [arXiv:2005.02877]。依標題它研究選擇性覆寫狀態記憶，能檢驗「Mem0 的操作集合以 IC-DST 為最近前例」與「本章計入的格式裡更多格式沒有修正入口」這兩句。
- **MultiWOZ 的兩個修正版本。**
  - 缺了哪一塊：兩個修正版各修了哪些標註、怎麼修。本章只從 2.2 的原文知道，2.1 修正了 40% 輪次中 32% 的狀態標註，並加了 slot 描述 [arXiv:2007.12720]。
  - 只在語料內成立的結論：〈同一個名字，不同的尺〉的版本效應只能從用這兩版的論文反推，IC-DST 與 D3ST 自己在各版之間的差距也無法歸到哪一類修正 [arXiv:2203.08568][arXiv:2201.08904]；未解問題 1 的「資料的更新型態太窄」。
  - 可以補：MultiWOZ 2.1: A Consolidated Multi-Domain Dialogue Dataset with State Corrections and State Tracking Baselines（arXiv 1907.01669，未讀）；MultiWOZ 2.4: A Multi-Domain Task-Oriented Dialogue Dataset with Essential Annotation Corrections to Improve State Tracking Evaluation（arXiv 2104.00773，未讀）。本章引用的 JGA 多半報在前一版上 [arXiv:2005.02877][arXiv:2402.10466]；IC-DST 的 Table 1（few-shot）與 Table 2（zero-shot 的逐領域 JGA）兩版都報，multi-domain 的 zero-shot 35.3 與全部消融只做在後一版上（消融用 100 段 dev 對話）[arXiv:2203.08568]。
- **schema 措辭的穩健性與「描述有沒有用」。**
  - 缺了哪一塊：SGD-X 的原論文，以及以示範取代描述的比較。
  - 只在語料內成立的結論：〈爭議〉的「自然語言描述到底有沒有用」只建立在 SGD、T5DST、DaP、D3ST 四篇上；未解問題 7 的 SS(JGA) 疑問無法回原論文核對。
  - 可以補：SGD-X: A Benchmark for Robust Generalization in Schema-Guided Dialogue Systems（arXiv 2110.06800，未讀）；Show, Don't Tell: Demonstrations Outperform Descriptions for Schema-Guided Task-Oriented Dialogue（arXiv 2204.04327，未讀），依標題它比較示範與描述，能檢驗「描述的價值集中在 schema 沒見過的情境」這一句。
- **以提示驅動大型 LM 的零樣本 DST。**
  - 缺了哪一塊：第二個子領域真正以提示驅動大型 LM 的只有 IC-DST 與 FnCTOD 兩篇；兩篇直接做這件事的候選以制式理由落選。
  - 只在語料內成立的結論：第二個子領域「LLM 時代」的敘事；累積派與重算派的分類；未解問題 7 的零曝光問題。
  - 可以補：ChatGPT for Zero-shot Dialogue State Tracking: A Solution or an Opportunity?（arXiv 2306.01386，未讀）；Towards LLM-driven Dialogue State Tracking（arXiv 2310.14970，未讀）；Dialogue Summaries as Dialogue States (DS2), Template-Guided Summarization for Few-shot Dialogue State Tracking（arXiv 2203.01552，未讀），IC-DST 以它當主要基線，依標題它把對話摘要當成狀態，能接起 DST 與摘要式記憶。
- **指令保持與多輪退化的基準。**
  - 缺了哪一塊：直接量「早先給的指令在後面是否仍被遵守」的論文，以及多輪指令遵循與多輪能力的基準；它們都沒進任何節點的候選池。
  - 只在語料內成立的結論：〈同一個 session 內的多輪退化與指令保持〉整節，計入的主證據只有 Lost in Conversation 一篇；「被提出的機制彼此沒有交叉檢驗」；未解問題 6 的指令保持。
  - 可以補：Measuring and Controlling Instruction (In)Stability in Language Model Dialogs（arXiv 2402.10962，未讀），依標題它量並控制對話中的指令穩定度；Multi-IF: Benchmarking LLMs on Multi-Turn and Multilingual Instructions Following（arXiv 2410.15553，未讀）；MultiChallenge: A Realistic Multi-Turn Conversation Evaluation Benchmark Challenging to Frontier LLMs（arXiv 2501.17399，未讀）；MT-Eval: A Multi-Turn Capabilities Evaluation Benchmark for Large Language Models（arXiv 2401.16745，未讀）。後三篇依標題都是多輪指令遵循或多輪能力的基準，能檢驗 Lost in Conversation 的發現在別的設定下是否成立。
- **記憶的時間知識圖。**
  - 缺了哪一塊：Zep 本身的論文。本章對 Zep 的了解只來自 Mem0 與 Hindsight 的表、Hindsight 的相關研究段落，以及廠商部落格 [arXiv:2504.19413][arXiv:2512.12818]。
  - 只在語料內成立的結論：〈記憶子領域內的爭議〉的「衝突與過時事實怎麼處理，六篇的做法沒有被互相比較過」與更新語意清單；Mem0、Zep、Hindsight 比較表的爭議。
  - 可以補：Zep: A Temporal Knowledge Graph Architecture for Agent Memory（arXiv 2501.13956，未讀）。
- **工具包層級的端對端評估。**
  - 缺了哪一塊：ConvLab-2 的論文。本章唯一「JGA 與任務成功率名次相反」的例子，來自它的 GitHub README，經 SUMBT 的精讀筆記轉述 [arXiv:1907.07421]。
  - 只在語料內成立的結論：〈端對端分數與 JGA 走勢不一致〉的 ConvLab-2 那一條；未解問題 8。
  - 可以補：ConvLab-2: An Open-Source Toolkit for Building, Evaluating, and Diagnosing Dialogue Systems（arXiv 2002.04793，未讀）。
- **跨 session 對話記憶的考場 MSC。**
  - 缺了哪一塊：MSC 的原始論文。本調研的筆記與未讀清單裡都沒有它，所以這裡不寫編號；DMR 的題型只能從 MemGPT 反推 [arXiv:2310.08560]。
  - 只在語料內成立的結論：MemGPT 在 DMR 上的結果與它的評審寬鬆度。
  - 可以補：MSC 的原始論文。
- **本節點的其餘候補。**
  - 缺了哪一塊：〈這一章回答什麼〉範圍說明列出的候補都沒讀。除了上面已列的遞迴摘要與 MultiWOZ 修正版，還有短期記憶管理、記憶指標、不綁值集合的多領域 DST、閱讀理解式 DST 與低資源遷移五篇。
  - 只在語料內成立的結論：第三個子領域的記憶維護只有六篇外部記憶；「彙整後放進乾淨上下文重問沒被測」；傳統 DST 的架構清單。
  - 可以補：Recursively Summarizing Enables Long-Term Dialogue Memory in Large Language Models（arXiv 2308.15022，未讀）；MemTool: Optimizing Short-Term Memory Management for Dynamic Tool Calling in LLM Agent Multi-Turn Conversations（arXiv 2507.21428，未讀）；Global-to-local Memory Pointer Networks for Task-Oriented Dialogue（arXiv 1901.04713，未讀）；Scalable Multi-Domain Dialogue State Tracking（arXiv 1712.10224，未讀）；Dialog State Tracking: A Neural Reading Comprehension Approach（arXiv 1908.01946，未讀）；MinTL: Minimalist Transfer Learning for Task-Oriented Dialogue Systems（arXiv 2009.12005，未讀）。
