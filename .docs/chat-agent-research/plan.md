...
邊的證據怎麼來的，要先講清楚：論文是依節點選進來的；某條邊有哪些證據，完全取決於精讀者有沒有在筆記的 `edges` 欄標上它。W3 沒有針對任何一條邊另外檢索。所以「某條邊缺什麼」只在本調研的語料內成立，E4 只有 3 篇就是這樣來的。
...
| **W3 綜合與驗證** | 一個節點的筆記約 30 萬字元，一個 agent 讀不完，所以每節點先分組（每組 ≤ 8 篇）寫段落草稿，再由一個 agent 組成一章；接著並行做兩件事：查核員先跑 [`tools/check_chapter.py`](tools/check_chapter.py) 把句子裡找不到出處的數字全部列出來，再做對抗式查核，另一個 agent 挑主張寫程式驗證；然後修訂、修訂後再跑一次數字比對、每章一個 critic。六條邊各一個 agent 分類哪些論文真的處理了那條資料流。最後寫 README，再由一個全域 critic 找跨章矛盾與漏收 | 實際 **79** |
| **W4 文字修訂** | W3 的章節 critic 與全域 critic 共找出約 180 條缺口，最多的一類是把「本調研沒讀到」寫成「領域裡沒有」。使用者決定這一輪只做文字修訂、不補讀：否定句收窄到本調研的語料，補上語料內的反例，critic 點名的論文先過 ID 閘門，再列進各章「已知缺口與未讀」（見〈誠實標記〉），驗證類缺口寫成程式。每章修訂後做對抗式稽核，最後同步 README 並做跨章一致性稽核 | 8×3 + 2 ≈ **26** |

針對性補讀延後：每篇都對應章節裡一句「領域裡沒有」的否定句，共 21 篇，由使用者在 2026-10-02 決定要不要做。清單列在各章的「已知缺口與未讀」。

合計約 **350 個 agent**。成本大頭在 W2。全域設定讓子代理預設跑 Haiku，W1 與 W1b 都是在這個預設下跑的；它們的轉抄數字與理由裡有捏造，可能與此有關，但沒有做過對照，無法確定。W2 起每個 agent 都明確指定 Opus。
...
每章的文獻表都標讀到的層級：✅ 全文精讀並通過錨點驗證，計入本節點 ／ 📖 讀完全文但不計入（精讀後判為低價值，或錨點未達門檻，原因寫在表裡）／ ❌ 只有摘要。只有 ✅ 計入 20 篇。W4 還有一種論文：critic 點名、通過 ID 閘門、但完全沒讀。這類論文集中列在各章的「已知缺口與未讀」小節；正文也可以提到，用來說明它能檢驗哪一句話，但一律寫成「標題（arXiv ID，未讀）」，只寫到標題層級。它們不進文獻表，不寫成 `[arXiv:]` 出處，也不引用它們的任何數字或結論。
