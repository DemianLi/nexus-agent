...
**契約層落地了，而且被用過。** `PluginRegistry` 有 15 個欄位（9 個折進 `createDeepAgent` 的註冊點 ＋ 6 條不折的通道；**2026-09-13 更新**：原文寫 14／5，第六條是 `feedback`，見 §2.1），`packages/` 底下 11 個生產 plugin 全走這條契約，`@nexus/plugin-echo` 靠 pnpm 相依隔離證明契約沒有偷偷要求伸手進組裝點。計劃書 §1 寫的形狀（命令式註冊、同層報錯、跨層遮蔽、fail-closed、載入期失敗）每一條都有對應的程式碼與測試。
