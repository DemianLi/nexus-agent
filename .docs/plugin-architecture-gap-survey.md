...
| 持久化：會話日誌 | **收完**（#172／#174，`--session-log <dir>`） | §五第 4 條 |
...
| `context/` 的 `session-reference`／`file-reference-local`（連同 seam `file-reference`） | **沒有結論**（2026-09-19）：web-app 出廠就掛；#215 判「不是缺口」的第 1 點只查了 base，第 2、3 點只講 `time-context` | §五第 5 條第 1 點 |
...
| `web`（搜尋與抓取） | **沒有結論**（2026-09-19）：dsh base 與三個 preset 出廠就掛，歸企業級站不住；照 dsh 自己寫的「網路政策更嚴的產品覆寫 `tool-web`」登記不掛，是一條現成的路，待拍板 | §三第 48 列 |
| `skill` 不在預設清單 | **沒有結論**（2026-09-19）：dsh base 與三個 preset 出廠就掛，`@nexus/plugin-skills` 從沒進過 `DEFAULT_PLUGINS` | §三第 38 列 |
| dsh 在對讀版本之後新增的五個目錄 | `browser-use`／`computer-use`／`ssh` 出廠不掛，照 #360 的判法；`deliverables`、`document` **沒有結論** | §三第 52–56 列 |
| goal 自動續行預設關（#180 §五） | **結論不動，前提待重核**（2026-09-19）：引的 dsh 原文沒錯，但 dsh base 出廠就掛續行驅動器 | §三第 18 列 |
