...
| 16 | `feedback` | 使用者對會話與 assistant 訊息的反饋 | **2026-09-15 回填**（原判「沒有」）：`@nexus/plugin-feedback` ＋ `registry.feedback`（第六條通道，[#278](https://github.com/DemianLi/nexus-agent/issues/278)→[#282](https://github.com/DemianLi/nexus-agent/pull/282)）——web 評得了每一輪、`/feedback` 記一則評語，只進日誌不進模型；[#283](https://github.com/DemianLi/nexus-agent/pull/283) 讓遙測能只在人送出回饋時補送會話日誌。形狀照 dsh 的 `message-feedback`／`command-feedback`，偏離登記在 `packages/nexus-core/src/feedback.ts` 檔頭：評分綁**輪**不綁訊息（[#267](https://github.com/DemianLi/nexus-agent/issues/267)）、沒有 `sessionId`、沒有 `list` | 有 |
