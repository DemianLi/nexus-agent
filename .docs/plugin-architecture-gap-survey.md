...
**調研日期**：2026-09-02。對讀版本：dsh `4e84901e6471b79ec0338099867ebb4606d12bb5`（master，2026-09-01 的 `dsh-0.1.2-alpha.4`）；Proteus `962304b320c57475227f056f52a71f3cd3d437f0`（main，2026-08-28，v0.3.0）；基座 `deepagents@1.13.1`。兩份 clone 都在 `references/`，不進版控。**2026-09-19 對 dsh `ddefc45fbc7f8e46dd73185e68295696d1297887`（master，2026-09-17，`release-dsh-0.1.6-alpha.2` 的合併）重核過 §三的套件清單與出廠組合**：九處更正逐處標了日期（`e2b`、`code-runtime`、`examples`、套件數、`web`、`TOOL_TIMEOUT`、`skill`、`context/` 那五個、goal 續行的前提），三處是後來過期、六處是寫下時就錯；清單的數法也改了，理由見「來源與可信度」那張表。
...
**它的存在理由是上下文成本**：本檔 61,427 字元（103KB）、24 個標題，每節平均 ~2,560 字元，（2026-09-16 更正：原文的「4,151 字元」是 bytes 誤標成字元，成因見 `development-plan.md` 同一處的更正；比例不受影響）而 §三那 51 列（2026-09-19 補到 56 列、劃掉兩列）的「狀態」欄只說**今天有沒有**，不說**判過沒有**——所以要知道一個「沒有」是缺口還是已經判過不做，最少得讀 4,000 字元。這張表把那個答案搬到零次額外讀取的位置。
...
| Proteus 的測量軸 | **不是缺口**；開圖條件已滿足，地圖是 #334 | §五第 7 條、§四 |
| 企業級與分散式 11 個套件 | **定位差異，不是缺口**（2026-09-19：原為 15，`e2b` dsh 已移除、`web` 移出；2026-09-26：`credentials`、`attachment` 拍板照 dsh 做、移出；`preset`、`extensions` 因 #714 拍板整套 Creator 模式，待 [#739](https://github.com/DemianLi/nexus-agent/issues/739) 重判） | §三小計 |
| `credentials`（憑證） | **照 dsh 做**（2026-09-26 拍板，[#717](https://github.com/DemianLi/nexus-agent/issues/717)，[#730](https://github.com/DemianLi/nexus-agent/issues/730)，未落地）；`authorization` 與 `deepseek-account` 這次不掛，之後會安排，登入對象是 nexus 自己的帳號 | §三第 11 列 |
| `attachment`（圖片與檔案附件） | **照 dsh 做**（2026-09-26 拍板，[#715](https://github.com/DemianLi/nexus-agent/issues/715)，未落地）：送圖那部分排在 #723 每會話換模型之後，gemma-4-31b 收圖實測先做（[#731](https://github.com/DemianLi/nexus-agent/issues/731)），量不過整張延後 | §三第 3 列 |
| `shell`／`subprocess`／`terminal`＋sandbox 行程隔離 | **決策 3 明文延後** | §三小計、§五第 7 條 |
| `jobs`／`schedule`／`workflow`／`session-query`／`workspace` | **需求未出現** | §三小計 |
| `web`（搜尋與抓取） | **不掛**（2026-09-19 拍板）：dsh base 與三個 preset 出廠就掛，歸企業級站不住；照 dsh 自己寫的「網路政策更嚴的產品覆寫 `tool-web`」登記不掛 | §三第 48 列；[`decisions-2026-09-19.md`](decisions-2026-09-19.md) 第 1 題 |
| `skill` 不在預設清單 | **進預設清單**（2026-09-19 拍板，[#440](https://github.com/DemianLi/nexus-agent/issues/440)，未落地）：dsh base 與三個 preset 出廠就掛，`@nexus/plugin-skills` 從沒進過 `DEFAULT_PLUGINS` | §三第 38 列；[`decisions-2026-09-19.md`](decisions-2026-09-19.md) 第 2 題 |
| dsh 在對讀版本之後新增的五個目錄 | `browser-use`／`computer-use`／`ssh` 出廠不掛，照 #360 的判法；`deliverables` **照 dsh 做**（2026-09-19 拍板：`present` [#441](https://github.com/DemianLi/nexus-agent/issues/441)、`workspace-changes` [#443](https://github.com/DemianLi/nexus-agent/issues/443)，未落地）；`document` **需求未出現**（2026-09-19 拍板） | §三第 52–56 列；[`decisions-2026-09-19.md`](decisions-2026-09-19.md) 第 3、8、11 題 |
| goal 自動續行預設關（#180 §五） | **serve 改預設開、CLI 維持預設關**（2026-09-19 前提重核完、拍板，[#445](https://github.com/DemianLi/nexus-agent/issues/445)，未落地）：引的 dsh 原文沒錯，但 dsh base 出廠就掛續行驅動器 | §三第 18 列；[`decisions-2026-09-19.md`](decisions-2026-09-19.md) 第 10 題 |
| 耐久檢查點（dsh `session-checkpoint-policy`） | **照 dsh 做**（2026-09-19 拍板，[#447](https://github.com/DemianLi/nexus-agent/issues/447)，未落地）：base 出廠就掛 | §三第 35 列；[`decisions-2026-09-19.md`](decisions-2026-09-19.md) 第 7 題 |
| core 的能力拆成可關的條目＋設定機制 | **啟動 [#46](https://github.com/DemianLi/nexus-agent/issues/46)**（2026-09-19 拍板），第一步讓摘要與修剪可以各自關掉（[#446](https://github.com/DemianLi/nexus-agent/issues/446)；剪刀仍包在摘要器外面，照 dsh 只在摘要開著時作用，所以是同一格兩個開關，不是兩顆 middleware） | [`decisions-2026-09-19.md`](decisions-2026-09-19.md) 第 12 題；[`srp-audit-2026-09-19.md`](srp-audit-2026-09-19.md) |

**§五的七條標題本身就帶結論**（「收完」／「判為不是缺口」／「認帳不做」／「登記不排」），所以那一節不需要這張表；這張表服務的是 **§三的各列**與 **§五第 7 條底下沒有標題的子項**。

**這張表最可能的錯法不是寫錯結論，是壓掉限縮**——原文刻意加的「但是」「只成立兩條半」「射程限於」在一格裡放不下。**一列讀起來比它指向的那一節更有把握時，那一列就是錯的。**

**這張表沒有絆索守著。** 照 §五第 7 條的慣例——掃自己散文的結構 gate 會永遠綠。改動任一節的結論時，同一張 PR 改這裡。
...
| dsh 頂層套件的對照（§三） | 主代理讀 | 第一手；套件清單出自 `ls -d packages/*/`，每列的「它做什麼」出自該套件 `README.zh.md` 的 `description:` 欄位。**2026-09-19 更正：這個數法會數到本機 clone 的未追蹤殘留。** 對讀版本 `4e84901` 上版控裡是 **50** 個頂層目錄，第 51 個 `examples` 在那個 SHA 上已經 0 檔（dsh 2026-08-26 `244de7c` 移除），是舊 checkout 留在磁碟上的目錄。清單改用 `git ls-files packages` 取頂層目錄；對 `ddefc45` 數是 54 |
...
**前兩版由子代理產出，兩版都整份丟掉了。** 第一版把 dsh 的套件數寫成 43（實際 51；**2026-09-19 更正**：51 也多數了一個，對讀版本上是 50，見上表）、把 `hooks/` 讀成「IDE 集成」（實際是執行使用者既有的 shell 鉤子）、抽核七個 issue 編號錯四個、compaction 那節的核心主張（「基座不知道自己壓過什麼」）與 `dist` 原始碼相反。第二版是刪掉錯的段落而不是修正，2.4 把 26 個套件塞成一句話，還留著我指出不存在的路徑。這一版是主代理重讀第一手材料寫的；沒把握的部分收在 §六，不混進正文。

## 一、結論

**契約層落地了，而且被用過。** `PluginRegistry` 有 16 個欄位（9 個折進 `createDeepAgent` 的註冊點 ＋ 7 條不折的通道；**2026-09-13 更新**：原文寫 14／5，第六條是 `feedback`，見 §2.1。**2026-09-21 再更新**：[#477](https://github.com/DemianLi/nexus-agent/issues/477) 把 `feedback` 那條收掉、換成 `services`（數字沒動），[#456](https://github.com/DemianLi/nexus-agent/issues/456) 加了第七條 `disabledEntries`——它是唯讀視圖不是註冊點，數字由 `apps/harness/src/registry-channel-count.test.ts` 在數），`packages/` 底下 11 個生產 plugin 全走這條契約，`@nexus/plugin-echo` 靠 pnpm 相依隔離證明契約沒有偷偷要求伸手進組裝點。計劃書 §1 寫的形狀（命令式註冊、同層報錯、跨層遮蔽、fail-closed、載入期失敗）每一條都有對應的程式碼與測試。

**廣度比 dsh 窄很多，但窄的地方分兩種。** dsh `packages/` 的 51 個頂層套件裡，我們有等價物的 10 個、部分的 14 個、沒有的 25 個、不適用的 2 個（**2026-09-05 更新過**：`guard` 與 `spill` 從「沒有」改成「部分」；**2026-09-15 再更新**：`feedback`、`goal` 改成「有」，`sandbox` 從「沒有」改成「部分」，理由見 §三小計）。25 個「沒有」裡 15 個是企業級與分散式的東西（`acp`／`sdk`／`typert`／`identity`／`settings`／`credentials`／`webhook`／`attachment`／`lsp`／`web`／`e2b`／`experimental`／`extensions`／`bundle`／`preset`）——那是定位差異，不是缺口。**2026-09-19 對 `ddefc45` 重算**：頂層目錄 54 個（原文的 51 多數了一個未追蹤殘留，對讀版本上是 50），有 9、部分 15、沒有 29、不適用 1；企業級剩 13 個——`e2b` dsh 已移除，`web` 在 base 出廠組合裡、原本就不該歸這一組（§三第 48 列）。**真正算缺口的是 agent 迴圈本身會用到、而 dsh 的 base 組合預設就開著的那幾個**：compaction 由誰選門檻、迴圈衛生的兩個 guard（重複呼叫提醒、單次工具逾時）、生命週期鉤子面（會話開始／提示詞提交／停止三個時刻——**2026-09-07 更正**：那是三個**格號**不是三個空缺，見 §五第 3 條）、以及狀態幾乎都只在記憶體裡。**這份清單 2026-09-05 被 [#146](https://github.com/DemianLi/nexus-agent/issues/146) 消耗掉大半**（十七張卡，逐張的產出與偏離索引在那張圖的結案留言）：前兩項收完（§五第 1、2 條）；狀態那項做掉**會話日誌**這一軸（[#172](https://github.com/DemianLi/nexus-agent/issues/172)、[#174](https://github.com/DemianLi/nexus-agent/issues/174)，CLI 與 `serve` 的 `--session-log <dir>`），checkpointer 仍是 `MemorySaver`、仍沒有 storage，但那兩軸今天零消費者，判過不做（[#155](https://github.com/DemianLi/nexus-agent/issues/155)，見決策 4 的補記——**原文寫的「決策 4 的三軸」是錯的**：那三軸問的都是「LangGraph 的狀態存在哪」，會話日誌不在其中任何一軸上）。~~**留下來的是生命週期鉤子面**（§五第 3 條，開圖條件部分滿足——面要不要做已經答了，掛在哪個縫上還沒有）**與等著它的 context 注入**（第 5 條）。~~ **2026-09-07 這兩項都不再開著**（[#212](https://github.com/DemianLi/nexus-agent/issues/212)）：那個「面」不是第十個東西，它就是 [#190](https://github.com/DemianLi/nexus-agent/issues/190) 的九格——dsh 自己的橋接把五個鉤子逐個訂在其中五格上——而五格逐格有結局；縫因此也定了（第 2 格），~~第 5 條剩下的是開卡時選射程~~——**那句 2026-09-07 又被推翻兩次**：射程不是獨立的選擇（[#216](https://github.com/DemianLi/nexus-agent/pull/216)），而整項判為不是缺口（#215）。~~**留下來的是第 6 條那半沒查的問題**（`ForkedSubAgent`）。~~ **2026-09-07 那半也查完了**：`ForkedSubAgent` 與 fork-mode 的 `CompiledSubAgent` 在型別上都進不到我們的註冊點，第 6 條判為認帳不做，那條窄由 `registry.test.ts` 的欄位絆索釘著。**這張表因此沒有「待驗」的項目了**——上面點名的四項全部有結局，還沒動的只剩第 5 條，而它 2026-09-07 開成 [#215](https://github.com/DemianLi/nexus-agent/issues/215) 並當天答完：**判為不是缺口，降到第 7 條登記不排**——base 的六個 `context/` 套件只掛一個，而那一個我們有等價物，`time-context` 那型的三個讀數逐個量過都沒有來源或不需要每步（第 5 條）。**這張表七條因此全部有結局。** shell／subprocess／terminal 與 sandbox 的行程隔離那半是決策 3 明文延後的，不算意外。
...
| 執行與工具 | 完整 | ✅ 有界 | MCP（#59）、fs 圍堵（#62）、QuickJS（#64）、「先讀後改」策略（#154，`packages/nexus-core/src/observation.ts`，照 dsh 的 `fs-observation-policy`，同樣由 `foldRegistry` 打底進 root 與每個 subagent）；shell 與行程沙箱決策 3 延後，且基座三條件互斥（`sandbox-backend-conflict.test.ts`）。檔案工具的圍堵模式與升級不在延後之列，2026-09-15 回填，見 §三第 31 列 |
| 反思與反饋 | 薄 | ⚠️ 有實作 | 輸出 schema 校驗：原本是 `@nexus/plugin-validation`（#73），**#252 之後也歸 `@nexus/core`**（`packages/nexus-core/src/output-schema.ts`，schema 隨工具註冊帶、fold 打底；plugin 只剩 re-export）。把「工具拋錯整場死」修回回饋的那一半 **#159 之後歸 `@nexus/core`**（`packages/nexus-core/src/containment.ts`，由 `foldRegistry` 打底進 root 與每個 subagent），不再是掛不掛隨人的 plugin |
...
清單出自 `ls -d references/deepseek-harness/packages/*/`。**2026-09-19 更正：那個數法把本機 clone 的未追蹤殘留也數進去了**（第 13 列 `examples`），對讀版本上版控裡是 50 個；之後改用 `git ls-files packages`。同一天對 `ddefc45` 重核：劃掉的第 12、13 列不計，第 52–56 列是 dsh 在對讀版本之後新增的目錄，編號接在最後、不插進字母序，免得既有的「§三第 N 列」引用失效——今天是 54 個。**判「出廠有沒有」時查的是 `packages/bundle/` 六個 bundle 加上 `packages/preset/agent-presets/presets/` 四個 preset**，只查 base 不夠（§五第 5 條第 1 點的更正）。「它做什麼」是該套件 `README.zh.md` 的 `description:` 欄位改寫成繁體、壓成一句。「狀態」四值：**有**（功能等價，形狀可以不同）／**部分**／**沒有**／**不適用**。
...
| 3 | `attachment` | 持久圖片附件 | 沒有。**2026-09-26 拍板照 dsh 做**（[#715](https://github.com/DemianLi/nexus-agent/issues/715)）：送圖那部分排在 #723 的每會話換模型之後；gemma-4-31b 在 NVIDIA 端點的收圖實測先做（[#731](https://github.com/DemianLi/nexus-agent/issues/731)），量不過整張改成延後。實作卡 [#732](https://github.com/DemianLi/nexus-agent/issues/732)、[#733](https://github.com/DemianLi/nexus-agent/issues/733)，未落地 | 沒有 |
...
| 7 | ~~`code-runtime`~~ `ptc-runtime` | 程式碼執行 seam：python、worker-thread 兩個提供方。**2026-09-19**：dsh 2026-09-12 改名 `ptc-runtime`（`7c9bb59`），提供方改成在全新受管 Node 行程、照已解析的沙箱政策跑 TypeScript 的 `ptc-runtime-node`，python 後端移進 `experimental/`；`ptc-runtime-node` 在 base 出廠掛（`packages/bundle/base/cordis.patch.yml:376`） | `@nexus/plugin-quickjs`：一個 JS 直譯器、走 custom tool。不是 seam | 部分 |
| 8 | `compaction` | 自動壓縮、按需 `/compact`、工具輸出修剪 | 基座無條件掛 `SummarizationMiddleware`。門檻與 subagent 射程已由 #142 收回（`createSummarizer` 同名取代 ＋ `foldSubAgents` 打底）；**工具結果修剪已由 #149 補上**，包在同名摘要器外面、壓力達標才剪、剪完可讓摘要那次模型呼叫整個不發生（射程 8,192–80,000 字元，上界是基座自己的 `toolTokenLimitBeforeEvict`）。**留痕已由 #143 補上**：一顆 `compaction/summary` 會話事件（切點、當時長度、歷史落點），root 與 subagent 各記在自己那份——dsh 的 `start`／`end` 兩顆湊不出來，因為基座只在成功走完之後才交出東西，硬記一顆 `start` 只能記在「我們猜它要壓了」的時間點。**溢出恢復已由 [#150](https://github.com/DemianLi/nexus-agent/issues/150)→[#166](https://github.com/DemianLi/nexus-agent/pull/166) 補上**：基座不做字串嗅探、認的是型別化的 `ContextOverflowError`，而我們的端點不在 `wrapOpenAIClientError` 比對的那四個字串裡，所以分類補在最靠近 adapter 的地方。**兩者都可以在組裝點關掉**（[#446](https://github.com/DemianLi/nexus-agent/issues/446)）：`summarization: false` 在 root、宣告的 subagent 與 general-purpose 各放一顆同名空殼，不再退回基座那組門檻；`toolResultPruning: false` 只拿掉剪刀、摘要照跑；CLI 與 serve 的入口等 #46。仍缺：`/compact`、指定範圍、壓縮鎖。修剪原文落日誌那條**理由換過一次**——不是「日誌不落盤」（2026-09-05 已落盤），是事件詞彙對不上 | 部分 |
| 9 | `context` | 不定義工具、每次請求加模型可見上下文：`agent-instructions`（AGENTS.md）、`time-context`、`file-reference`、`file-reference-local`、`session-reference`、`tmux-context`（**六個**，原本這列漏了 `file-reference-local`） | `@nexus/plugin-agent-instructions` ≈ `agent-instructions`（2026-09-18 照 dsh 新做、預設掛著，[#388](https://github.com/DemianLi/nexus-agent/issues/388)；在那之前這格寫的是 `@nexus/plugin-memory`，而它既不同形、也不在預設清單上）。其餘五個沒有，而**那五個 base 一個都沒掛**——2026-09-07 判為不是缺口，降到 §五第 7 條。**2026-09-19 更正：base 沒掛不等於出廠沒掛。** web-app 掛了 `session-reference` 與 `file-reference-local`（連同 seam `file-reference`），那三個的判定見 §五第 5 條第 1 點的更正。**2026-09-19 拍板**（[`decisions-2026-09-19.md`](decisions-2026-09-19.md) 第 5、6 題）：等 web 要做 @ 提及／@file 補全時再一起做，web 先 | 部分 |
| 10 | `core` | 會話日誌、系統提示詞組裝、工具註冊表、agent 詞彙與預設迴圈、scope | `session-log.ts`（會話日誌）、`registry.tools`＋`fold.ts`＋`base-tools.ts`（工具註冊與呈現順序）、迴圈外包 deepagents、系統提示詞由基座組而 `harness-profile.ts`（#141）守著它。沒有 `ctx` 那棵 context 樹（那是 Cordis 的），但**依名字提供／取得服務有了**——`registry.services`（[#459](https://github.com/DemianLi/nexus-agent/issues/459)），缺件是載入失敗而不是 Cordis 那種等待 | 有 |
| 11 | `credentials` | 憑證引用 seam：設定裡只放引用不放值、按操作解析、環境與檔案提供方 | 沒有 seam。`docs/standards.md:19-25` 是規範（一律環境變數、不得有 fallback），不是機制。**2026-09-26 拍板照 dsh 做**（[#717](https://github.com/DemianLi/nexus-agent/issues/717)）：憑證服務、每人一份只有自己讀得到的受管憑證檔、兩層 `.env` 擋危險名字，實作卡 [#730](https://github.com/DemianLi/nexus-agent/issues/730)，未落地。同一個目錄的 `authorization`（登入取憑證；dsh 寫的適用條件是「憑證沒辦法透過設定提供時」）與 `deepseek-account`（DeepSeek 帳號平台登入）這次不掛，demian 原話：「這個流程之後會安排，只是登入的不是 deepseek 的帳號，是屬於 nexus 自己的」 | 沒有 |
| 12 | ~~`e2b`~~ | 遠端 Linux 沙箱（fs＋subprocess） | 沒有 | ~~沒有~~ **不計**：dsh 2026-09-11 已移除（`c49db8b`，「retire remote execution providers」） |
| 13 | ~~`examples`~~ | 示範 | `apps/harness/src/spike/`、`*.fixture.ts` | ~~不適用~~ **不計**：**這一列從一開始就不該在**——對讀版本 `4e84901` 上 `packages/examples` 已經 0 檔（dsh 2026-08-26 `244de7c` 移除），`ls -d` 數到的是本機 clone 的未追蹤殘留 |
...
| 16 | `feedback` | 使用者對會話與 assistant 訊息的反饋 | **2026-09-15 回填**（原判「沒有」）：`@nexus/plugin-feedback` ＋ `registry.feedback`（第六條通道，**2026-09-20 [#477](https://github.com/DemianLi/nexus-agent/issues/477) 更正：那條通道收掉了**，改由 `registry.services` 的 `messageFeedback` 這個名字提供——通道數因此從十六降到十五；[#278](https://github.com/DemianLi/nexus-agent/issues/278)→[#282](https://github.com/DemianLi/nexus-agent/pull/282)）——web 評得了每一輪收尾那則回覆、`/feedback` 記一則評語，只進日誌不進模型；[#283](https://github.com/DemianLi/nexus-agent/pull/283) 讓遙測能只在人送出回饋時補送會話日誌。形狀照 dsh 的 `message-feedback`／`command-feedback`；**2026-09-18 [#382](https://github.com/DemianLi/nexus-agent/issues/382)**：評分改指名訊息 id（同 dsh，原本綁**輪**）、補上 `list`（重新整理後畫得回來、重播的回覆評得了），剩下的偏離只有沒有 `sessionId`，登記在 `packages/nexus-core/src/feedback.ts` 檔頭 | 有 |
...
| 18 | `goal` | 每會話一個持久目標：域、模型工具、使用者命令、自動續行 | `@nexus/plugin-goal`（#128、#129、#177）＋ `apps/harness/src/goal-driver.ts`。**2026-09-15 回填**（原判「部分」，理由是四個子套件缺 `goal-round-driver`，#152 登記的空缺）：四個都有了——續行排程器由 [#180](https://github.com/DemianLi/nexus-agent/issues/180)→[#181](https://github.com/DemianLi/nexus-agent/pull/181) 落地（目標沒達成時自己再開一輪），[#183](https://github.com/DemianLi/nexus-agent/pull/183) 讓模型自己收掉目標後停手、[#184](https://github.com/DemianLi/nexus-agent/pull/184) 讓命令列壓得住輪次上限。**載體不同**：dsh 的 driver 是一個 plugin，我們的輪迴圈歸入口點所有、`PluginRegistry` 沒有一條通道排得出一輪，所以住在 `apps/harness`，偏離登記在該檔檔頭；`agent.steer()` 沒有等價物（`agent/turn-stopping` 那格只佔一半，見 §五第 3 條）。**2026-09-19：[#180](https://github.com/DemianLi/nexus-agent/issues/180) §五「兩個入口預設關」的前提要重核。** 它引的那句「goal 是狀態而非調度器——自動續行是需要你刻意掛載的可選消費方」**是 dsh 當時 `packages/goal/README.zh.md:12` 的原文，引得沒錯**；但同一時間 dsh base 已經出廠掛著 `goal-round-driver`（對讀版本 `4e84901` 上就在，今天在 `packages/bundle/base/cordis.patch.yml:302`），所以「可選」說的是套件可以不掛，不是出廠關著。dsh 擋自動續行的是另一格：續行的啟用狀態只存在行程裡、每次 `agent/created` 都會歸零，resume 或 fork 之後要使用者明確授權（`packages/goal/goal/README.zh.md:99`、`packages/goal/goal-round-driver/README.zh.md:57`）；它的 README 現在也改寫成「自动续行必须单独启用」。**預設關這個決定不動**：CLI 那半的理由（`HEADLESS_APPROVALS` 是確定性拒絕，續行會空轉）不受影響，~~serve 那半的前提待重核~~ **serve 那半 2026-09-19 重核完、拍板改成預設開**（[`decisions-2026-09-19.md`](decisions-2026-09-19.md) 第 10 題，[#445](https://github.com/DemianLi/nexus-agent/issues/445)）：serve 要核准時停下來等人，不會空轉。同一句引文在 `README.md`、`apps/harness/src/cli.ts`、`apps/harness/src/goal-driver.ts`、`apps/harness/src/serve.test.ts` 各有一處，都指回這裡，#445 落地時一併改 | 有 |
| 19 | `guard` | 迴圈衛生：`repeat-tool-reminder`（同參數重複呼叫 3／5／8 次時建議性提醒）、`timeout-policy`（單次工具呼叫協作式逾時→清楚的模型錯誤）。**dsh base 兩個預設開著** | **兩個行為都有了，載體沒有。** 重複提醒 [#147](https://github.com/DemianLi/nexus-agent/issues/147)→[#157](https://github.com/DemianLi/nexus-agent/pull/157)（`repeat-reminder.ts`，投遞掛在 `beforeModel`）；超時措辭 [#162](https://github.com/DemianLi/nexus-agent/issues/162)→[#176](https://github.com/DemianLi/nexus-agent/pull/176)（`containment.ts`，主詞是「等了多久」，判別式 `error.name === 'TimeoutError'` 分得開使用者按的取消）。**`@nexus/plugin-guard` 這個載體是一筆偏離登記**：基座自己就武裝截止時間（`defaultConfig:{timeout}` → `AbortSignal.timeout`），一個只剩措辭的 plugin 沒東西可武裝，而照 [#159](https://github.com/DemianLi/nexus-agent/issues/159) 的結論，圍堵旁邊的行為藏在選配 plugin 裡等於沒有。~~仍缺 `TOOL_TIMEOUT` 分類碼（三個消費者一個都不在，刻意不發）~~ **2026-09-19 更正：這個碼從 [#270](https://github.com/DemianLi/nexus-agent/pull/270)（2026-09-12）起就在發**，見 §五第 2 條的更正 | 部分 |
| 20 | `hooks` | 在 agent 運行期執行使用者**既有的** Claude Code／Codex `hooks.json` shell 鉤子：會話開始、提示詞提交、工具前後、停止時觸發；可帶模型可見訊息阻塞、附加上下文、強制繼續 | 沒有。近似物：`wrapToolCall`／`wrapModelCall`（工具、模型前後）、`registry.approvals`（阻塞工具）、`lifecycle.onDispose`（關機）。**2026-09-07 更正**（[#212](https://github.com/DemianLi/nexus-agent/issues/212)）：原文寫「缺會話開始、提示詞提交、停止三個時刻」——那三個是**格號**不是空缺，兩格佔住、一格判過不是缺口（見 §五第 3 條）。這一列判「沒有」**照舊成立**，但理由只剩一條：**沒有跑外部 shell 鉤子的引擎**，而要不要做那個引擎就是 §五第 3 條的 (b)。**2026-09-16 (b) 答完了：不做**（[#360](https://github.com/DemianLi/nexus-agent/issues/360)）——**dsh 的六個出廠 bundle 一個都沒掛 bridge**，照它的實際做法就是不放進出廠組合；而「可掛但不出廠」我們沒有 profile／patch 層可以表達（#46）。這一列因此從「下一張圖」變成**登記不排，帶三條重開條件**（見 (b)） | 沒有 |
...
| 23 | `interaction` | 人機協作：`commands`、`permission-presets`、`tool-ask-user`、`user-approval`、`user-questions` | `commands` ✅（`@nexus/plugin-commands`，形狀照 `dsh-commands`）；`user-approval` ≈ `registry.approvals`＋`approval.ts`＋`approval-card.tsx`（**2026-09-08 量過語意差**，[#220](https://github.com/DemianLi/nexus-agent/issues/220)：提問側對得上，**應答側不是掛點、結果詞彙窄一格、審計事件零顆**，逐條見 §六第 7 條，結局在 §五第 7 條）。`permission-presets` 沒有；`tool-ask-user` ✅（`@nexus/plugin-ask-user`，模型面五個欄位照抄 dsh 的工具 schema）＋ 折疊器那側的判別式（[#231](https://github.com/DemianLi/nexus-agent/issues/231) 第一刀，2026-09-09）。**`user-questions` 這個能力接縫本身沒有**：dsh 是一條獨立的 Cordis waterfall（`ctx.userQuestions`），可以有多個 answerer 競標；我們只有一顆 `interrupt()`，退成「一條通道＋一個 `kind` 判別式」，所以「誰來回答」不是可組裝的——**唯一的應答者就是接在線那頭的人**。一手證據見 [`structured-input-survey.md`](structured-input-survey.md) | 部分 |
...
| 29 | `preset` | 按會話從 preset 檔組裝 agent；persona | 沒有。~~組裝來源是程式碼不是檔案（#140 卡裡已對照）~~ **2026-09-26**：組裝來源已是 YAML（[#454](https://github.com/DemianLi/nexus-agent/issues/454)），但出貨清單仍是一份套用到每個會話；[#714](https://github.com/DemianLi/nexus-agent/issues/714) 拍板整套 Creator 模式，這一列要重判（盤點 T-29），決策卡 [#739](https://github.com/DemianLi/nexus-agent/issues/739) | 沒有 |
...
| 31 | `sandbox` | 行程沙箱 seam：隔離、各平台後端（bwrap／Landlock／Seatbelt／Windows ACL）、策略解析 | **2026-09-15 回填**（原判「沒有」）：**策略與升級那半有了，行程隔離那半仍沒有。** 模式三格照 dsh 的 `SandboxMode`（`read-only`／`workspace-write`／`danger-full-access`，`packages/nexus-core/src/sandbox.ts`），作用在檔案工具的 fence 上（[#238](https://github.com/DemianLi/nexus-agent/issues/238) 那組）：[#240](https://github.com/DemianLi/nexus-agent/pull/240) 逐次解析並用提示句告訴模型（對 dsh `sandbox-policy`）、[#241](https://github.com/DemianLi/nexus-agent/pull/241) 執行期 `/sandbox` 切得動並記進會話日誌、[#249](https://github.com/DemianLi/nexus-agent/pull/249) 被擋時請得到一次只蓋一個檔的升級（對 dsh `sandbox/src/escalation.ts`；另開一顆工具是登記過的載體偏離，見 `apps/harness/src/sandbox-escalation.ts` 檔頭）、[#289](https://github.com/DemianLi/nexus-agent/pull/289) 讓 grant 綁住被擋下的那一次、[#332](https://github.com/DemianLi/nexus-agent/pull/332) 讓子代理照委派當下的模式判。各平台的行程隔離後端（bwrap 那一類）沒有，`@nexus/plugin-quickjs` 檔頭的結構性偏離照舊；決策 3 延後的是這一半 | 部分 |
...
| 35 | `session` | 持久會話資料平面：persistence seam（jsonl／sqlite）、checkpoint 策略、投影、標題、統計、對外遙測 | `session-log.ts`（append-only）、**`session-store.ts`＋`session-persistence.ts`＋`jsonl-session-store.ts`（persistence seam，#172／#174：CLI 與 `serve` 的 `--session-log <dir>`，選擇性、無預設路徑）**、`session-telemetry.ts`＋coordinator＋`@nexus/plugin-telemetry-otel`（#100）、`sessions.ts`／`session-registry.ts`（#138 subagent 各一份）。sqlite provider／checkpoint-policy／projection／title 沒有。**2026-09-19 拍板**（[`decisions-2026-09-19.md`](decisions-2026-09-19.md) 第 7、9 題）：日誌照 dsh 預設落盤到 `~/.nexus-agent/sessions`（[#444](https://github.com/DemianLi/nexus-agent/issues/444)）；checkpoint-policy 照 dsh 在模型請求與工具呼叫前 flush（[#447](https://github.com/DemianLi/nexus-agent/issues/447)） | 部分 |
| 36 | `settings` | 使用者設定 seam ＋ YAML／JSON 檔提供方 | 沒有（#46） | 沒有 |
| 37 | `shell` | bash／pwsh 本地與沙箱執行器、持久與一次性工具 | 沒有。決策 3 延後 | 沒有 |
| 38 | `skill` | 由提供方發現、經會話目錄與 skill 工具載入的可複用指令 | `@nexus/plugin-skills`（#69）靠基座 `createSkillsMiddleware`。沒有 `tool-skill`、`skill-badge`。**2026-09-19 更正：它不在 `DEFAULT_PLUGINS` 裡，也從沒進過**（`apps/harness/src/cli.ts` 的歷史裡 `createSkillsPlugin` 零筆）——零設定的 CLI 與 serve 上 skills 不存在，跟第 9 列記的 `@nexus/plugin-memory` 同一型錯（套件在、產品路徑上不在）。dsh 出廠就掛：base 的 `skill`／`skill-filesystem`／`tool-skill`（`packages/bundle/base/cordis.patch.yml:280-290`，對讀版本上就在），以及 standard／ptc／cordis 三個 preset 的 `skill-filesystem`／`tool-skill`。要不要進預設清單 ~~沒有結論~~ **2026-09-19 拍板進預設**（[`decisions-2026-09-19.md`](decisions-2026-09-19.md) 第 2 題，[#440](https://github.com/DemianLi/nexus-agent/issues/440)） | ~~有（薄）~~ 部分 |
| 39 | `spill` | 過大的工具文字外溢到儲存、回傳可檢索定位 | **基座已經占住這個 seam，而且在產品路徑上**（[#151](https://github.com/DemianLi/nexus-agent/issues/151) 實測）：`createFilesystemMiddleware` 在 80,000 字元之上把工具結果寫進 `/large_tool_results/<callId>.txt`，換成頭尾預覽加一句 `read_file` 指路，取回路徑實測通。原文寫的「`truncateArgsSettings` 是截斷不是外溢」對照錯了對象——那是另一個機制。[#170](https://github.com/DemianLi/nexus-agent/issues/170)→[#171](https://github.com/DemianLi/nexus-agent/pull/171) 補掉基座在寫入失敗時把原文一起丟掉的缺陷。仍缺：門檻調不動也關不掉、非文字區塊被吃掉（後者 #642 已解）。**2026-09-26 補**：續接或 serve 重開後預覽指的路徑讀不到；拍板照 dsh 分兩步補（[#734](https://github.com/DemianLi/nexus-agent/issues/734)、[#719](https://github.com/DemianLi/nexus-agent/issues/719)），搜尋工具截斷另開 [#735](https://github.com/DemianLi/nexus-agent/issues/735) | 部分 |
...
| 47 | `util` | 共享工具函式 | — | 不適用 |
| 48 | `web` | 搜尋／抓取服務、提供方後端、模型側工具 | 沒有。**2026-09-19 更正：原本歸「企業級」站不住，而且寫下時就錯了。** dsh base 出廠就掛 `web`／`web-search-deepseek`／`web-fetch-http`／`tool-web`（`packages/bundle/base/cordis.patch.yml:458-476`，對讀版本 `4e84901` 上 `tool-web` 就在）；web-app 把 base 那列關掉、改由 preset 掛，standard／ptc／cordis 三個 preset 都掛 `tool-web`。base 的註解自己寫了一條給網路受限部署的路：「products with a stricter network policy override tool-web」（`:447-449`）——內網部署照這句登記不掛。~~沒有結論~~ **2026-09-19 拍板不掛**（[`decisions-2026-09-19.md`](decisions-2026-09-19.md) 第 1 題） | 沒有 |
...
| 52 | `browser-use` | 瀏覽器操作能力：選擇並註冊一個瀏覽器後端（2026-09-12 新增） | 沒有。**dsh 出廠也不掛**：`apps/` 與 `packages/bundle/`、`packages/preset/` 搜 `dsh-browser-use` 零筆，引用它的只有 `packages/experimental/` 的實驗性後端（2026-09-19） | 沒有 |
| 53 | `computer-use` | 桌面操作能力：選擇並註冊一個桌面提供方（2026-09-12 新增） | 沒有。出廠也不掛，同第 52 列 | 沒有 |
| 54 | `deliverables` | 記錄一輪交給使用者的東西：`tool-present`（模型用 `present` 宣告交付檔案）、`workspace-changes`（每個頂層輪次改了哪些檔：git 工作樹快照＋檔案工具編輯前後的整檔捕獲）（2026-09-14 新增） | 沒有。**出廠就掛**：`present` 在 standard／ptc／cordis 三個 preset（standard 的 `agent.cordis.yml:261`），`workspace-changes` 在 web-app（`packages/bundle/web-app/cordis.patch.yml:300`，配 `ui-deliverables` 的改動檔卡片）。~~沒有結論~~ **2026-09-19 拍板照 dsh 做**（[`decisions-2026-09-19.md`](decisions-2026-09-19.md) 第 3、8 題）：`present` [#441](https://github.com/DemianLi/nexus-agent/issues/441)、`workspace-changes` [#443](https://github.com/DemianLi/nexus-agent/issues/443)，兩張的 web 卡片交 dev-ui | 沒有 |
| 55 | `document` | Host 端 Office 轉換與可重用的 PDF 結果（`office-to-pdf`）（2026-09-16 新增） | 沒有。web-app 出廠掛（`packages/bundle/web-app/cordis.patch.yml:228`），供 web 右側欄的文件預覽；我們的 web 沒有這個面。~~沒有結論~~ **2026-09-19 拍板不做，登記「需求未出現」**（[`decisions-2026-09-19.md`](decisions-2026-09-19.md) 第 11 題） | 沒有 |
| 56 | `ssh` | POSIX SSH 提供方家族：共享連線、遠端檔案系統、受管子行程、檔案效果沙箱（2026-09-11 新增） | 沒有。出廠也不掛，同第 52 列 | 沒有 |

**小計（2026-09-15 更新）**：有 10（`core`、**`feedback`**、`fs`、**`goal`**、`mcp`、`plan`、`runtime-diagnostics`、`skill`、`todo`、`test-support`）；部分 14（`api`、`boot`、`client`、`code-runtime`、`compaction`、`context`、`guard`、`host`、`interaction`、`llm`、**`sandbox`**、`session`、`spill`、`subagent`）；沒有 25；不適用 2。**2026-09-15 回填三列**：`feedback` 沒有→有、`goal` 部分→有、`sandbox` 沒有→部分，理由各在該列。**`guard` 與 `spill` 是 2026-09-05 從「沒有」改過來的**——前者兩個行為都落地了只是載體不同，後者基座本來就占著而我們一直對照錯了機制。**2026-09-19 對 `ddefc45` 重算（54 個目錄，劃掉的第 12、13 列不計）**：有 9（`skill` 移出）；部分 15（`code-runtime` 改名 `ptc-runtime`；`skill` 移入）；沒有 29（`e2b` 劃掉，補第 52–56 列）；不適用 1（`util`）。

**25 個「沒有」怎麼分**（2026-09-15：原為 27，`feedback` 與 `sandbox` 移出）：企業級與分散式 15 個（`acp`、`sdk`、`typert`、`identity`、`settings`、`credentials`、`webhook`、`attachment`、`lsp`、`web`、`e2b`、`experimental`、`extensions`、`bundle`、`preset`）——定位差異，**原文寫「14 個」但列了 16 個名字，而且把 `api`／`host`／`client` 三個「部分」的算了進去，一併改正**（2026-09-05；16 個裡的 `feedback` 2026-09-15 落地，剩 15）；決策 3 明文延後 3 個（`shell`、`subprocess`、`terminal`；原本的第 4 個 `sandbox` 策略與升級那半已落地、移到「部分」，行程隔離那半仍延後）；需求未出現 5 個（`jobs`、`schedule`、`workflow`、`session-query`、`workspace`）；~~**agent 迴圈自己會用到、dsh base 預設開著、我們沒有的 2 個：`hooks`、`storage`**~~ **→ 1 個：`storage`**（2026-09-16 更正，[#360](https://github.com/DemianLi/nexus-agent/issues/360)）。（原本是 3 個，`guard` 已於 2026-09-05 落地；`session` 那列的 persistence 那半也落地了。）**`hooks` 從這一組移出：它不在 dsh base，也不在任何出廠 bundle**——`packages/bundle/` 六個 bundle 全目錄搜 `hook` 只有 4 筆，全是 React hook 與 test hook；唯一掛 bridge 的是 `snapshots/*/cordis.yml` 的 `- insert:`，而那份 patch 自稱只修「deterministic test composition」。**`storage` 這一半是對的**：`packages/bundle/base/cordis.patch.yml:141-155` 掛了 `storage`／`storage-json`／`storage-domain` 三個條目。兩者的結局因此同形但理由不同：**`hooks` 判不做，理由是 dsh 出廠也不掛**（§五第 3 條 (b)）；**`storage` 判不做，理由是零消費者**（[#155](https://github.com/DemianLi/nexus-agent/issues/155)，結論不動，重開條件照舊）。**2026-09-19 改成 29 個「沒有」**：企業級與分散式 **13** 個（扣掉 dsh 已移除的 `e2b`；`web` 移出——它在 base 出廠組合裡，第 48 列）；決策 3 明文延後 3 個；需求未出現 5 個；零消費者判不做 1 個（`storage`）；dsh 出廠也不掛 4 個（`hooks`，#360；`browser-use`／`computer-use`／`ssh` 照同一判法）；~~**沒有結論 3 個**（`web`、`deliverables`、`document`）~~。**2026-09-19 拍板後**（[`decisions-2026-09-19.md`](decisions-2026-09-19.md)）：`document` 併進需求未出現（變成 6 個）；`web` 照 dsh 自己寫的覆寫路登記不掛（1 個）；`deliverables` 拍板照 dsh 做（#441、#443），落地前仍算「沒有」（1 個）。總數仍是 29。**2026-09-26 拍板後**：`credentials`（[#717](https://github.com/DemianLi/nexus-agent/issues/717)）與 `attachment`（[#715](https://github.com/DemianLi/nexus-agent/issues/715)）拍板照 dsh 做，從企業級那一桶移出（剩 11 個），跟 `deliverables` 同一類：拍板照 dsh 做、落地前仍算「沒有」，這一類變成 3 個。總數仍是 29。剩下 11 個裡的 `preset`（第 29 列）與 `extensions`，因 [#714](https://github.com/DemianLi/nexus-agent/issues/714) 同日拍板整套 Creator 模式要重判（盤點 T-29，[#739](https://github.com/DemianLi/nexus-agent/issues/739)），判完再調這一桶的數。
...
排序準則：**agent 迴圈自己會碰到**（不是部署面、不是分散式）× **dsh base 預設就開著**（表示它認為每個 harness 都該有）× **我們的基座表達得出來**（表達不出來的要標偏離）× **大小**。企業級那 15 個不排（原文寫 14，§三小計 2026-09-05 改正時漏了這一處；2026-09-15 `feedback` 落地後剩 15）；決策 3 延後的 3 個（加上 `sandbox` 的行程隔離那半）登記不排。
...
- ~~**`TOOL_TIMEOUT` 分類碼刻意不發**：dsh 給 retry／sandbox／replay 路由用，我們三個消費者一個都不在。~~ **2026-09-19 更正：這個碼從 [#270](https://github.com/DemianLi/nexus-agent/pull/270)（2026-09-12）起就在發。** `classifyThrownToolError` 把 `TimeoutError` 標成 `TOOL_TIMEOUT`（`packages/nexus-core/src/containment.ts:240`），經 `settle` 寫進會話日誌那顆 `tool/result` 的 `error`（`:403-407`）——目的地是會話日誌，不是 dsh 講的 retry／sandbox／replay，那三個用途今天仍然沒有。這句話寫下時（[#176](https://github.com/DemianLi/nexus-agent/pull/176)，2026-09-05）是對的，是後來過期；`containment.ts` 檔頭同一句話與 §三第 19 列在同一張 PR 改掉。
...
| --- | --- | --- | --- |
| `SessionStart` | `agent/created` | `:202` | 第 1 格 |
| `UserPromptSubmit` | `agent/pre-step` | `:218` | 第 2 格 |
| `PreToolUse` | `tools/pre-execute` | `:237` | 第 4 格 |
| `PostToolUse` | `tools/post-execute` | `:246` | 第 7 格 |
| `Stop` | `agent/turn-stopping` | `:269` | 第 3 格 |

**2026-09-16 重量**（[#360](https://github.com/DemianLi/nexus-agent/issues/360)，對 `0d1f50007f9bca3f52b06e1c3074fa14d5fb0720`）：**第 1 列的事件不是改行為，是整個被併掉了。** `agent/session-start` 在現行 dsh 的 `*.ts` **零命中**，只活在 `.agents/notes/archived/`（dsh 自己的規則：archived 是凍結的，不是現行權威）；`packages/core/agent/src/runtime-types.ts` 對 `session-start` 零命中，而那正是舊表引的檔。`docs/event-producer-consumer.zh.md` 已無該列，只剩 `agent/created` — **`serial`** — 而本條點名的四個消費者全部列在它底下。併過去的那一格**是 awaited 且失敗回滾創建**（`packages/core/agent/src/index.ts:547` 的 `await this.ctx.serial(…, 'agent/created', …)`，同檔 `:417`）。其餘四列只是行號漂移。

`hooks-codex` 訂**同樣五個**（`:184`／`:198`／`:224`／`:233`／`:259`；2026-09-16 重量，原記 `:188`／`:199`／`:225`／`:234`／`:260`）。dsh 明寫「『原生钩子』不是一个包——原生钩子只是一个普通的 Cordis 插件，订阅规范的生命周期事件」，`packages/hooks/` 三個套件是**橋接**，把使用者現成的 CC／Codex shell `hooks.json` 翻譯到那個介面。**所以「生命週期鉤子面」不是一個要另建的面，它就是那九格。**

五格逐格的結局（`apps/harness/src/interception-index.test.ts` 是那份索引，#190 的結案留言是逐格的核對過程）：

| 格 | 時刻 | 現況 |
| --- | --- | --- |
| 1 | `agent/session-start` | [#201](https://github.com/DemianLi/nexus-agent/issues/201) 判過**不是缺口**：dsh 那格是 `mode: 'emit'`、明著寫「a deliberate gap」；權限最弱＋消費者零＋時刻以 `SessionRegistry` 的建構存在（`session-registry.ts:169` 的 `new SessionLog(...)`；#190 記的 `:170` 已漂一行），三條缺一不可。**2026-09-16 更新**（#360）：**第一條的前提沒了**——`agent/session-start` 這個事件已被併進 serial 且 awaited 的 `agent/created`，「emit-only、擋不住啟動」在原始碼上反過來了，#201 引的那句 JSDoc「This is a notification, not a veto」也隨之消失。**結論 (c) 不動，理由從三條變兩條**：消費者零 ＋ 時刻以建構存在。之所以不重推，是因為會讓第 2 條倒的只有「做外部 hooks 引擎」，而那件事 2026-09-16 判為不做（見 (b)） |
...
- **(b) 要不要執行外部 `hooks.json`（dsh 兩個 bridge 做的事）——2026-09-16 答完了：不做**（[#360](https://github.com/DemianLi/nexus-agent/issues/360)）。

  **理由不是「零消費者」，是照 dsh 的實際做法就是不做。** 判準是 AGENTS.md 的技術實現標準，而套用它之前要先問對問題：dsh 是「有這個套件」還是「出廠就掛」。量完是前者——`packages/bundle/` 六個 bundle（`base`／`web-app`／`headless`／`sdk-app`／`sdk-minimal`／`acp-app`）**沒有一個掛 bridge**（全目錄搜 `hook` 只有 4 筆，全是 React hook 與 test hook）。唯一掛它的是 `snapshots/*/cordis.yml`，用 `- insert:`，而那份 patch 的檔頭自己寫著「**The shipped profile owns application startup; this patch fixes only the deterministic test composition**」——是測試為了覆蓋 bridge 自己插的。`python/sdk-runtime/package.json` 那兩筆是相依宣告，不是掛載。**dsh 的實際做法因此是：寫出來、發布得出去、出廠零掛載，要用的人自己寫一行 patch。**

  **而「可掛但不出廠」這個狀態我們今天表達不出來**：沒有 profile／patch 層（§三第 4 列；[#46](https://github.com/DemianLi/nexus-agent/issues/46) 明文「需要時再啟動」），所以一個不進 `DEFAULT_PLUGINS` 的 bridge 等於沒有人開得起來。這是偏離規則的標準情形——基礎建設表達不出來，最接近的實作就是不做。我們自己的樹本來就記著這件事：`packages/nexus-plugin-validation/README.md:36` 寫著「轉接使用者外部 `hooks.json` 的橋接，而**橋接不在出廠組合裡**」。

  **開卡時記的三條價錢，2026-09-16 對 `0d1f5000` 重量過，兩條變了。** 留著是因為重開時還要用：
  1. ~~`hooks-claude-code/src/index.ts:205` 帶著 `TODO(session-start-gating)`，注入是 detached、best-effort，**可能錯過第一個請求**（#201 量過）。~~ **不成立了**：TODO 全樹零命中，而且機制補上了——`ctx.on('agent/created', …)`（`:202`）末尾 `await run`（`:215`），而 `agent/created` 是 serial、awaited、失敗回滾創建。**首輪投遞現在擋得住。**
  2. **那個瑕疵不只在 session-start** —— **一半成立**：`SubagentStart`（`:283`，原記 `:284`）與 `SubagentStop`（`:293`）仍是**不 await** 的 `detached.track`。
  3. **兩個橋接的覆蓋不對稱** —— **照舊成立**：`hooks-codex` 只訂五個，沒有 subagent 那兩格。

  **照第 7 條的慣例不掛絆索**——要守的宣稱是「dsh 的 bundle 沒掛它」，那是上游的東西，掃 `references/` 的結構 gate 守不住上游改名或搬檔（同 `context/` 那五個套件的處置）。**改為明著寫重開條件，任一成立就重開**：(1) **dsh 把 bridge 放進任一出廠 bundle**；(2) **#46 的 profile／patch 層落地**，那時「可掛但不出廠」表達得出來，第二條理由消失；(3) **有人手上有 `hooks.json` 而且要拿到 nexus 上跑**（照第 6 條的一般判法）。

  **#215 移交過來的那筆帶外注入殘值（`subagent/start`）因此維持認帳**，持有人仍是這一條。

**subagent 的兩個時刻，這張表跟 #190 都漏了**：`subagent/start`（`hooks-claude-code:280`）／`subagent/end`（`:290`；2026-09-16 重量，原記 `:281`／`:291`）。**兩張圖都漏是因為來源本身沒列**——#190 的九格出自 dsh 的設計筆記 `2026-06-30-interception-extension-points.zh.md`，那份筆記 grep `subagent` **零命中**。逐個的處置見下：

- **`subagent/end` 判為不是缺口**，理由是**消費者一個都不存在**，與第 9 格（`tools/result`）同型：dsh 那格是 `mode: 'emit'`、只觀測；我們的 `SessionEventType` 十種（`session-log.ts:71-81`）**一顆 subagent 事件也沒有**，而更近的那個缺席（工具事件）已經婉拒過一個指名道姓的消費者（#180 第五節，理由在 `goal-driver.ts` 檔頭）。**2026-09-12 起工具事件有了**（[#264](https://github.com/DemianLi/nexus-agent/issues/264)），那個類比少了一半；這一格的判決靠的是「消費者一個都不存在」，那一條照舊成立。
...
`time-context`（現在時間、時區、經過時長）是 dsh `context/` 裡最小的一個；`agent-instructions` 我們有（`@nexus/plugin-agent-instructions`，2026-09-18 起；[#388](https://github.com/DemianLi/nexus-agent/issues/388) 之前這裡寫的是 `@nexus/plugin-memory`，那是錯的）。dsh 讓注入的上下文以 user 角色訊息進會話歷史，所以可回放、可壓縮——**那一點也照抄了**，基線記成 `user/message`。
...
1. **排序準則第二條不成立。** §五的準則第二條是「dsh base 預設就開著」，而 `packages/bundle/base/package.json` 的 83 個相依裡，**六個 `context/` 套件只有 `agent-instructions` 在內**（`grep -nE 'dsh-(time-context|agent-instructions|file-reference|session-reference|tmux-context)'` 只回 `:120`）。

   **⚠️ 這一條當年接著寫「而那一個我們有等價物」，那半句 2026-09-18 被 [#388](https://github.com/DemianLi/nexus-agent/issues/388) 推翻。** `@nexus/plugin-memory` 包的是基座的 `createMemoryMiddleware`，跟 `agent-instructions` 差在兩處模型看得到的地方（它每次都附一段叫模型主動改記憶檔的 `<memory_guidelines>`，dsh 的模板寫的是反過來那句；載體是 system prompt 而不是會話日誌裡的 `user/message`），**而且它根本不在 `DEFAULT_PLUGINS` 裡**——產品路徑上等於不存在。當年只核了「有沒有讀 `AGENTS.md`」，沒核**模型看到什麼**。今天的等價物是 `@nexus/plugin-agent-instructions`（#388 照 dsh 新做、預設掛著）。~~**上面那五個的判定不受影響**：它們本來就不在 base 裡，下面兩條理由各自成立。~~ **2026-09-19 更正：五個裡有三個出廠就掛，只是不在 base。** web-app 掛了 `session-reference`（`packages/bundle/web-app/cordis.patch.yml:75`）與 `file-reference-local`（`:78`），後者相依的 seam `file-reference` 跟著 web-app 出貨（web-app 的 `package.json:103-104`）；對讀版本 `4e84901` 上這兩列就已經在。下面第 2、3 點講的都是 `time-context`，所以這一項「不是缺口」的判定**只對 `time-context`、`tmux-context` 成立**，另外三個 ~~沒有結論~~ 2026-09-19 拍板：等 web 要做 @ 提及／@file 補全時再一起做（[`decisions-2026-09-19.md`](decisions-2026-09-19.md) 第 5、6 題）。

   **2026-09-16 同一條準則第二次失手，對象是 `hooks`**（[#360](https://github.com/DemianLi/nexus-agent/issues/360)）。§三小計把它寫成「dsh base 預設開著」的兩個之一，而 **dsh 的六個出廠 bundle 一個都沒掛 bridge**。兩次的成因一樣：**「dsh 有這個套件」被當成「dsh 出廠就開」**，而那是兩件事——dsh 發布得出去的套件遠多於任何 profile 掛起來的。**下次用這條準則之前先查 `packages/bundle/`，不要查 `packages/`。**

   **2026-09-19 同型的錯又出了兩次，方向相反。** 前兩次是把「dsh 有這個套件」讀成「出廠就開」；§三第 48 列的 `web`（歸成企業級）與上面那句「本來就不在 base 裡」，是把「base 沒有」讀成「出廠沒有」。上一句的規矩因此要補一半：**`packages/bundle/` 要查完六個 bundle（web-app 把 agent 平面整批關掉、改由 preset 掛），再加上 `packages/preset/agent-presets/presets/` 的四個 preset；只查 `base` 不夠。**
...
- `shell`／`subprocess`／`terminal`，以及 `sandbox` 的行程隔離那半（策略與升級那半 2026-09-15 回填為已落地，見 §三第 31 列）：決策 3 明文延後直到容器方案明朗；基座三條件互斥（`sandbox-backend-conflict.test.ts`）讓 QuickJS 走了 custom tool。
- `jobs`／`schedule`／`workflow`／`extensions`／`session-query`／`workspace`：dsh 有、我們沒有、需求沒出現。
- **`context/` 那五個非預設的套件**（`time-context`／`file-reference`／`file-reference-local`／`session-reference`／`tmux-context`）：**2026-09-07 從第 5 條降下來的**（[#215](https://github.com/DemianLi/nexus-agent/issues/215)），理由三條見第 5 條。**2026-09-19 縮成兩個**：`session-reference`、`file-reference-local`（連同 seam `file-reference`）是 web-app 出廠掛的，見第 5 條第 1 點的更正；這一列的「登記不排」與下面兩條重開條件**只對 `time-context`、`tmux-context` 成立**，另外三個 2026-09-19 拍板等 web（見第 5 條第 1 點）。第六個 `agent-instructions` 是 base 唯一掛的，**而我們的等價物 2026-09-18 才真的落在產品路徑上**（[#388](https://github.com/DemianLi/nexus-agent/issues/388)；在那之前這裡認的是 `@nexus/plugin-memory`，它既不同形也沒掛）。**這一列照第 7 條的慣例不掛絆索**——一個掃空的結構 gate 會永遠綠（紀律見第 3 條末），**改為明著寫重開條件，任一成立就重開**：(1) **wire 上出現 client 時區**（`apps/web` 送上來、`@nexus/wire` 的 protocol 收得下），那時「按用戶時區解釋未限定的時間」才有來源；(2) **有 plugin 指名要每步新鮮的讀數**（不只是時鐘），那時第 5 條第 2 題的鉤子選擇才值得付那一格 `beforeModel` 節點。
- **核准的應答者掛點與審計事件**：**2026-09-08 從 §六第 7 條降下來的**（[#220](https://github.com/DemianLi/nexus-agent/issues/220)）。**兩半的理由完全不同，所以分開寫。**
  - **應答者掛點——不做，而理由是 dsh 自己寫的。** 那條 waterfall 是**兩個提問者**逼出來的：`tools/pre-execute` 的 `ask` 決策，以及沙箱拒絕後的一次性升級重試。被否掉的替代方案講得更死——內聯進 ACP 橋不行，因為「无法服务第二个发起方（沙箱升级发生在执行开始之后，没有 pre-execute 时刻）」（`.agents/notes/implemented/feature/2026-07-06-approval-seam.zh.md:97`）。**我們今天只有一個提問者**（`nexus-plugin-plan-mode/src/index.ts:525`，`exit_plan_mode` 回 `ask`），沒有沙箱升級也沒有鉤子橋，**逼出可組合 seam 的那件事在我們樹上還沒發生**。順帶：dsh 也否掉過 `registerProvider()`（同檔 `:96`），所以「單一提供方」不是它沒想到，是它想過不要。**2026-09-10 更新：沙箱升級現在有了，但它沒有變成第二個提問者。** [#238](https://github.com/DemianLi/nexus-agent/issues/238) 第 2 項的載體是另開一顆工具（甲），升級是**那顆工具在 pre-execute 的 `ask`**——騎在既有的那一個提問者上（listener 現在是三個：plan-mode、`submit_record`、升級，三個都是 pre-execute 那一種）。dsh 的第二個提問者之所以逼得出 seam，是因為它「發生在執行開始之後」，而我們沒有這種提問者。所以這一條的結論照舊，**變的是前提那半句「沒有沙箱升級」**。
...
6. **dsh `docs/subsystems/README.zh.md` 列的 53 個子系統頁與 51 個套件目錄的對應**（2026-09-19 更正：套件目錄對讀版本上是 50，今天 54，見 §三開頭）——我以套件目錄為對照單位，沒有以子系統頁再對一次（例如 `agent-team` 在 `experimental/`、`token-meter` 在 `llm/`、`scope` 在 `core/`）。
