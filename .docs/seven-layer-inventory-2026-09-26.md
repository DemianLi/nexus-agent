...
- L-23　插話（next-step steer）（盤點後開 #710；2026-09-26 拍板照 dsh 做（harness 先、web 後））
...
- G-17　互動式憑證取得（dsh authorization：問人取得憑證的 flow 註冊表）（盤點後開 #717，與 G-09 同一張；2026-09-26 拍板這次不掛，之後會安排（登入對象是 nexus 自己的帳號））
...
- L-26※　模型選擇：部署預設模型＋每會話換模型與推理強度（盤點後開 #723；2026-09-26 拍板先做型錄 #729，每會話選擇晚點）
- O-06　token 用量（含小項：快取讀寫欄位）（#574、#517；盤點後開 #724；2026-09-26 拍板晚點做，重開條件寫在卡上）
- O-13　日誌格式版本與舊格式（含小項：日誌遷移形狀）（#507；盤點後在 #507 留言補充）
- O-16　會話查詢與搜尋（sessionQuery：清單、精確讀、標題、全文搜尋、lineage）（#610、#631、#632；盤點後開 #725 管投影快取，2026-09-26 拍板先量、門檻 1 秒，量測在 #742；其餘各項已有去處；#631 內容搜尋 2026-09-26 拍板照 dsh 出廠預設關）
- V-02　就緒檢查（模型就緒＋任務就緒 preflight）（#436；盤點後在 #436 留言補充；#436 2026-09-26 拍板暫緩）
- G-10　子行程環境清洗（MCP stdio 與 git 快照；合併小項「MCP 環境絆索」）（#461；盤點後開 #726；2026-09-26 拍板 HTTP 代理照 dsh 整套補，另開 #746）
...
| E-02 | 三種沙箱模式＋執行期切換 | **完成** | 有 | 有-預設 | #238、#241、#478、#437（2026-09-26 #437 拍板 `/permission` 取代 `/sandbox`，排後面） |
...
| C-05 | 大輸出外溢（spill） | **部分** | 09-19 判「有」；第一輪判「部分」但理由錯（把 tool-text 的傳輸截斷當成外溢）；調研筆記 §三第 39 列本來就判「部分」 | 部分 | #151、#170、#538、#642；盤點後開 #719；拍板後另開 #734、#735、#736 |
| C-06 | 手動 /compact | **判過不做** | 判過：登記為偏離（#142） | 沒有 | #142 |
| C-07 | 圖片省略（image-offload）與圖片不送模型（#642） | **待拍板**（改判） | 判過：attachment 屬定位差異 | 沒有（附件儲存、read_image、image-offload 都沒有）；圖片不送模型已照 dsh「沒掛附件」那一格落地（#642）。附件本身在 #642 寫「另議」、在 web 清單第 20 項寫「先 grilling」，是未決，不是判過不做 | #642；盤點後開 #715；2026-09-26 拍板照 dsh 做：#731 先量，實作 #732、#733 |
...
| C-16※ | 系統提示詞的身分與 persona（部署方可設的前後綴、{{model}}／{{cwd}}）（新） | **部分** | — | 部分 | 盤點後開 #720；2026-09-26 拍板三句用英文、開頭句不自稱寫程式 |
...
| L-13 | 子代理：續行／背景（continuable）、send_message、list_agents | **缺口** | 已登記：#324「要做時再開卡」 | 沒有 | 盤點後開 #708；2026-09-26 拍板照 dsh 做、預設開，改由地圖 #737 追蹤 |
| L-14 | 子代理：逐個選模型 | **缺口** | 已登記：#328 第 3 項 | 沒有 | #328；盤點後開 #709，#328 第 3 項是另一件；2026-09-26 拍板照一般缺口排、低優先，等型錄 #729 |
...
| L-23 | 插話（next-step steer）（新） | **缺口** | （新） | 沒有 | 盤點後開 #710；2026-09-26 拍板照 dsh 做 |
| L-24 | 串流閒置逾時（吐了內容之後停住）（新） | **完成** | （新） | 有-預設 | #521 |
| L-25 | 每步平行工具呼叫上限（新） | **缺口** | （新，09-19 沒列） | 沒有 | 盤點後開 #711 |
| L-26※ | 模型選擇：部署預設模型＋每會話換模型與推理強度（新） | **部分** | — | 部分 | 盤點後開 #723；2026-09-26 拍板先做型錄 #729，每會話選擇晚點 |
...
| O-03 | OTel 遙測（會話日誌外送） | **判過不做** | 判過（#279，偏離一） | 有-選配 | #279、#477；2026-09-26 拍板照 dsh 拿掉 `full`（全部送出）模式：#750 |
| O-04 | 模型失敗分類碼進日誌（含清單缺口 5；併入：每次重試與其失敗原因進日誌） | **缺口** | 缺口 | 沒有 | #434；盤點後開 #712 管重試進日誌那一半，帶碼那一半在 #434 |
| O-05 | 遙測 ops：agent-error 記錄（含清單缺口 6） | **dsh 也沒有** | 缺口 | 沒有 | #435；盤點後在 #435 留言，寫明前提已被推翻；2026-09-26 拍板關卡（wontfix），重開條件寫在卡上 |
| O-06 | token 用量（含小項：快取讀寫欄位） | **部分** | 部分；快取欄位未判（小） | 部分 | #574、#517；盤點後開 #724；2026-09-26 拍板晚點做，重開條件寫在卡上 |
...
| O-16 | 會話查詢與搜尋（sessionQuery：清單、精確讀、標題、全文搜尋、lineage）（新） | **部分** | 判過：需求未出現（調研筆記第 34 列，併在會話匯出那列） | 有-僅 serve | #610、#631、#632、#633；盤點後開 #725 管投影快取（2026-09-26 拍板先量、門檻 1 秒，量測在 #742），其餘各項已有去處；#631 內容搜尋 2026-09-26 拍板照 dsh 出廠預設關 |
| O-17 | 組裝設定可查（--dump-config）（新） | **待拍板**（改判） | （09-19 沒有這一列） | 有-預設（只有 --dump-config，CLI 與 serve 都收；沒有 schema dump，也沒有不含使用者層的 dump） | #46、#454；盤點後開 #716；2026-09-26 拍板兩種都做：#740、#741 |
...
| V-02 | 就緒檢查（模型就緒＋任務就緒 preflight） | **部分** | 有（模型就緒）；自動化的任務就緒檢查沒有 | 部分：模型就緒是 repo 外的人工盤點，做過三次（2026-08-28、08-29、09-04），結果以靜態清單留在 repo。survey.ts 的 16 個候選停在 08-29，還列著 09-03 已下架的 gpt-oss-120b，09-04 那輪只剩 9 個可用；清單已過期，也沒有東西會讓它紅。repo 沒有探測程式。自動化的 preflight 沒有，因為沒有真模型的 CI job，#436 內文也沒寫缺 secret 就硬失敗 | #436；盤點後在 #436 留言補充；#436 2026-09-26 拍板暫緩 |
...
| V-06 | 持續回歸：真模型、定期（併 09-19 清單缺口 7） | **缺口** | 缺口（清單第 7 項） | 沒有 | #436；盤點後在 #436 留言補充；2026-09-26 拍板暫緩，重開條件寫在卡上 |
...
| G-10 | 子行程環境清洗（MCP stdio 與 git 快照；合併小項「MCP 環境絆索」） | **部分** | 部分：結果比 dsh 嚴，但全靠上游、零絆索；未判（小） | 部分 | #461；盤點後開 #726；2026-09-26 拍板 HTTP 代理照 dsh 整套補，另開 #746 |
...
| G-17 | 互動式憑證取得（dsh authorization：問人取得憑證的 flow 註冊表）（新） | **待拍板** | — | 沒有 | 盤點後開 #717，與 G-09 同一張；2026-09-26 拍板這次不掛，之後會安排 |
...
- 下面 Phase 表 Phase 4 那一列把 #435（`agent-error`）列為 Phase 定義外的後續。本報告 O-05 已判定它的前提不成立；#435 已於 2026-09-26 拍板關閉（wontfix）。
...
| Phase 4 — HITL + 可觀測性 + 反思（約 3 個 PR） | 完成。驗收三句都改寫過並附可執行證據：interrupt-resume（PR #71）、工具失敗回饋（PR #73），tracing 披露有 tracing.ts；圍堵與輸出校驗後來搬進 core（#159、#252）。 | 文件沒有列剩項。Phase 定義外的後續：#434；#435 已於 2026-09-26 關閉（wontfix，見 O-05）。 |
