...
- **沒有卡的 3 條缺口**：L-13、L-23、L-25。L-23 的前提已經變了。#190 當初不補插話，理由是「沒有消費者」；如今送出佇列（L-22）已經落地，09-25 拍板佇列時也寫了「插話另開」，但一直沒開卡。
- **本次新找到兩條「部分」**，都沒有卡：
...
- 推翻：O-05 `agent-error`。09-19 只讀了協調器裡的 relay，沒讀生產者。dsh 只在 capture `live` 時掛 relay，出廠後端是 on-demand，所以出廠路徑上一筆都不發。**#435 的前提不成立，triage 時要改判。**
...
- T-04　逐 agent 工具 allow/deny（#328）
- T-06　MCP resources（列出／讀取 server 資源）（#430）
- T-07　MCP server instructions 進系統提示詞（#431）
- C-03　摘要 prompt 保留原始意圖與使用者糾正（清單缺口 3）（#432（OPEN，needs-triage））
- L-13　子代理：續行／背景（continuable）、send_message、list_agents（無卡）
- L-14　子代理：逐個選模型（#328）
- L-23　插話（next-step steer）（無卡）
- L-25　每步平行工具呼叫上限（無卡）
- O-04　模型失敗分類碼進日誌（含清單缺口 5；併入：每次重試與其失敗原因進日誌）（#434）
- V-06　持續回歸：真模型、定期（併 09-19 清單缺口 7）（#436）
- G-05　具名權限預設（沙箱＋核准捆成一組；合併清單缺口 8）（#437）

**已拍板未落地 — 5**

- T-11　skills 進預設清單（#440、決議 09-19 第 2 題）
- T-23　模型看得到的工具 schema 目錄＋新鮮度 gate（#442、決議 09-19 第 4 題）
- C-11　跨會話引用（session-reference，清單第 13 項）（decisions-2026-09-19 第 5 題、調研筆記第 9 列）
- C-12　@file 引用補全（file-reference-local，清單第 14 項）（decisions-2026-09-19 第 6 題、調研筆記第 9 列）
- L-09　長期目標＋自動續行（goal-driver）（#180、#445、#638）

**待拍板 — 5**

- T-28　plugin 開發者工具（tool-cordis、cordis-inspect-providers、tool-plugin-manager）（無卡）
- C-07　圖片省略（image-offload）與圖片不送模型（#642）（#642）
- O-17　組裝設定可查（--dump-config）（#46、#454）
- G-09　憑證儲存（不寫進行程環境）（無卡）
- G-17　互動式憑證取得（dsh authorization：問人取得憑證的 flow 註冊表）（無卡）

**部分 — 14**

- E-08　程式碼執行（#615、#503）
- T-20　會話標題（含 LLM 生成標題 session-title-llm）（無卡）
- C-05　大輸出外溢（spill）（#151、#170、#538）
- C-16※　系統提示詞的身分與 persona（部署方可設的前後綴、{{model}}／{{cwd}}）（無卡）
- L-03　模型請求重試（含串流內與串流中段錯誤）（#516、#520、#545）
- L-07　崩潰孤兒輪補結（`interrupted` closer）（#306）
- L-08　耐久檢查點（session-checkpoint-policy）（#447、#599）
- L-26※　模型選擇：部署預設模型＋每會話換模型與推理強度（無卡）
- O-06　token 用量（含小項：快取讀寫欄位）（#574、#517）
- O-13　日誌格式版本與舊格式（含小項：日誌遷移形狀）（#507）
- O-16　會話查詢與搜尋（sessionQuery：清單、精確讀、標題、全文搜尋、lineage）（#610、#631、#632）
- V-02　就緒檢查（模型就緒＋任務就緒 preflight）（#436）
- G-10　子行程環境清洗（MCP stdio 與 git 快照；合併小項「MCP 環境絆索」）（#461）
- G-15　人工監督（核准面板、計劃評審、提問）（#408、#409）
...
| E-08 | 程式碼執行 | **部分** | 部分；偏離已登記 | 有-選配 | #615、#503 |
...
| T-04 | 逐 agent 工具 allow/deny | **缺口**（改判） | 已登記：#328 第 3 項（needs-triage） | 沒有：沒有逐 agent 的允許／拒絕遮罩；rootOnly 是 dsh 逐工具呼叫者拒絕（DELEGATED_CALLER 一類）的宣告化，樁仍可見，不算這一格 | #328 |
| T-05 | MCP 工具（接外部 MCP server） | **完成** | 有，同形 | 有-選配 | — |
| T-06 | MCP resources（列出／讀取 server 資源） | **缺口** | 缺口（09-19 清單缺口 1） | 沒有 | #430 |
| T-07 | MCP server instructions 進系統提示詞 | **缺口** | 缺口（09-19 清單缺口 2） | 沒有 | #431 |
| T-08 | MCP prompts（提示詞模板） | **dsh 也沒有** | 同形 | 沒有 | — |
| T-10 | web_search／web_fetch | **判過不做** | 未判（09-19 清單第 9 題） | 沒有 | 決議 09-19 第 1 題 |
| T-11 | skills 進預設清單 | **已拍板未落地** | 未判（09-19 清單第 10 題） | 有-選配 | #440、決議 09-19 第 2 題 |
...
| T-20 | 會話標題（含 LLM 生成標題 session-title-llm） | **部分**（改判） | 部分；LLM 標題未判（小） | 部分：70357bb 只有規則回退 fallbackThreadTitle（列表冷讀時推，同 dsh fallbackSessionTitle）；LLM 首句標題沒有，已由 #650（OPEN、ready-for-agent，2026-09-26 demian 拍板拆卡）追蹤 | — |
| T-21 | 斜線命令 | **完成** | 有 | 有-預設 | — |
...
| T-28 | plugin 開發者工具（tool-cordis、cordis-inspect-providers、tool-plugin-manager）（新） | **待拍板** | （09-19 只在工作筆記記下 cordis preset 的 tool-cordis，未判） | 沒有 | — |
...
| C-03 | 摘要 prompt 保留原始意圖與使用者糾正（清單缺口 3） | **缺口** | 缺口（表格列＋清單第 3 項，兩側都核過） | 沒有 | #432（OPEN，needs-triage） |
| C-04 | 工具輸出修剪（tool-result-pruner） | **完成** | 有 | 有-預設 | #149、#446、#514 |
| C-05 | 大輸出外溢（spill） | **部分** | 09-19 判「有」；第一輪判「部分」但理由錯（把 tool-text 的傳輸截斷當成外溢）；調研筆記 §三第 39 列本來就判「部分」 | 部分 | #151、#170、#538、#642 |
| C-06 | 手動 /compact | **判過不做** | 判過：登記為偏離（#142） | 沒有 | #142 |
| C-07 | 圖片省略（image-offload）與圖片不送模型（#642） | **待拍板**（改判） | 判過：attachment 屬定位差異 | 沒有（附件儲存、read_image、image-offload 都沒有）；圖片不送模型已照 dsh「沒掛附件」那一格落地（#642）。附件本身在 #642 寫「另議」、在 web 清單第 20 項寫「先 grilling」，是未決，不是判過不做 | #642 |
...
| C-11 | 跨會話引用（session-reference，清單第 13 項） | **已拍板未落地** | 未判（表格列＋清單第 13 項） | 沒有 | decisions-2026-09-19 第 5 題、調研筆記第 9 列 |
| C-12 | @file 引用補全（file-reference-local，清單第 14 項） | **已拍板未落地** | 未判（表格列＋清單第 14 項） | 沒有；觸發條件已到：2026-09-26 拍板 Q1–Q9，#651（harness 列檔路由＋提示句，ready-for-agent）與 #653（web 的 @ 選單，等 #651）已開卡，尚未合併；追蹤要補 #651、#653 | decisions-2026-09-19 第 6 題、調研筆記第 9 列 |
...
| C-16※ | 系統提示詞的身分與 persona（部署方可設的前後綴、{{model}}／{{cwd}}）（新） | **部分** | — | 部分 | — |
...
| L-13 | 子代理：續行／背景（continuable）、send_message、list_agents | **缺口** | 已登記：#324「要做時再開卡」 | 沒有 | — |
| L-14 | 子代理：逐個選模型 | **缺口** | 已登記：#328 第 3 項 | 沒有 | #328 |
...
| L-23 | 插話（next-step steer）（新） | **缺口** | （新） | 沒有 | — |
| L-24 | 串流閒置逾時（吐了內容之後停住）（新） | **完成** | （新） | 有-預設 | #521 |
| L-25 | 每步平行工具呼叫上限（新） | **缺口** | （新，09-19 沒列） | 沒有 | — |
| L-26※ | 模型選擇：部署預設模型＋每會話換模型與推理強度（新） | **部分** | — | 部分 | — |
...
| O-04 | 模型失敗分類碼進日誌（含清單缺口 5；併入：每次重試與其失敗原因進日誌） | **缺口** | 缺口 | 沒有 | #434 |
| O-05 | 遙測 ops：agent-error 記錄（含清單缺口 6） | **dsh 也沒有** | 缺口 | 沒有 | #435 |
| O-06 | token 用量（含小項：快取讀寫欄位） | **部分** | 部分；快取欄位未判（小） | 部分 | #574、#517 |
...
| O-13 | 日誌格式版本與舊格式（含小項：日誌遷移形狀） | **部分** | 部分；形狀不同、未登記成偏離（小） | 部分 | #507 |
...
| O-16 | 會話查詢與搜尋（sessionQuery：清單、精確讀、標題、全文搜尋、lineage）（新） | **部分** | 判過：需求未出現（調研筆記第 34 列，併在會話匯出那列） | 有-僅 serve | #610、#631、#632、#633 |
| O-17 | 組裝設定可查（--dump-config）（新） | **待拍板**（改判） | （09-19 沒有這一列） | 有-預設（只有 --dump-config，CLI 與 serve 都收；沒有 schema dump，也沒有不含使用者層的 dump） | #46、#454 |
...
| V-02 | 就緒檢查（模型就緒＋任務就緒 preflight） | **部分** | 有（模型就緒）；自動化的任務就緒檢查沒有 | 部分：模型就緒是 repo 外的人工盤點，做過三次（2026-08-28、08-29、09-04），結果以靜態清單留在 repo。survey.ts 的 16 個候選停在 08-29，還列著 09-03 已下架的 gpt-oss-120b，09-04 那輪只剩 9 個可用；清單已過期，也沒有東西會讓它紅。repo 沒有探測程式。自動化的 preflight 沒有，因為沒有真模型的 CI job，#436 內文也沒寫缺 secret 就硬失敗 | #436 |
...
| V-06 | 持續回歸：真模型、定期（併 09-19 清單缺口 7） | **缺口** | 缺口（清單第 7 項） | 沒有 | #436 |
...
| G-05 | 具名權限預設（沙箱＋核准捆成一組；合併清單缺口 8） | **缺口** | 已知缺口（sandbox-mode.ts:21-27 登記「還沒做」，沒有卡） | 沒有 | #437 |
...
| G-09 | 憑證儲存（不寫進行程環境） | **待拍板**（改判） | 判過：定位差異（筆記 §三小計的 credentials） | 沒有（09-19 那次「判過：定位差異」沒有決議出處，而且與同一份筆記移出 web 的判準相衝突） | — |
| G-10 | 子行程環境清洗（MCP stdio 與 git 快照；合併小項「MCP 環境絆索」） | **部分** | 部分：結果比 dsh 嚴，但全靠上游、零絆索；未判（小） | 部分 | #461 |
...
| G-15 | 人工監督（核准面板、計劃評審、提問） | **部分**（改判） | 有 | 有-僅 serve（核准與提問面板完成；計劃評審仍走核准卡，#652／#654 已拍板未落地） | #408、#409 |
| G-16 | 合規 | **dsh 也沒有** | 框架要、dsh 也沒有 | 沒有 | decisions-2026-09-25 G3 |
| G-17 | 互動式憑證取得（dsh authorization：問人取得憑證的 flow 註冊表）（新） | **待拍板** | — | 沒有 | — |
