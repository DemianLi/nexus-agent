...
- 驗收：瀏覽器完成「提問 → 看事件流 → 核准工具 → 收結果」全迴圈（[#79](https://github.com/DemianLi/nexus-agent/pull/79) 已閉合）；eval 有可比較的通過率數據，且該數據足以讓模型定案 —— **前半有了，後半仍然沒有，但擋住的東西換了**。[#83](https://github.com/DemianLi/nexus-agent/pull/83) 記的是「階梯的底板太高」，而受控的底板在 [#84](https://github.com/DemianLi/nexus-agent/pull/84) 降到 20B（總量）、五階仍然全部滿分，同一組判準在 11B 上量得出 `0.19`。所以真正的落點是**判準在 11B 有鑑別力、在 20B 以上飽和** —— 20B 以上的選型只剩成本與延遲兩個軸有解析度，品質那一軸沒有。**Phase 5 因此仍不宣告完成**，README 那條「完成一個 Phase 跳 minor」不適用。
