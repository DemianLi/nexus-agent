...
| 判準對照 | `llama-11b` | 11B ／ 不詳 | 4（失敗 `rejected`×7、`budget`×1） | 0.58（0–1.00） | 0.53（0–0.83） | 1.25（0–3） | **0.00** | 14734（3070–27072） |
    ...
    → **同一輪暴露出選中的那個模型跑不完基準任務，而它是七題全跑才看得見的。** `oss-120b` 42 次執行裡 **21 次被端點回 `429 Too Many Requests`**，而且**斷點跟題目綁定**：三條簡單題 18/18 全通過，`reverse-round-trip` 與 `grep-across-files` **6/6 全滅**，`edit-after-read` 從第 3 次開始斷。**單獨冷開機重跑一次（沒有前面那 42 次暖身）逐格重現**：同樣的斷點、同樣那兩條全滅、簡單題的 token 平均 5045 對 5049。所以它既不是隨機節流，也不是「前面的執行把配額打滿」。**其餘 147 次執行零 429**，所以也不是整把 key 的吞吐上限 —— 範圍窄到這一個 id。→ **直接後果有兩個。**①上一輪那張表裡 `oss-120b` 的 `1.00` 是**只由簡單題撐起來的**（20 個判分裡 18 個來自簡單題），這一輪起改標「樣本不足」而不是分數。②**選型的失敗模式那一軸現在指向反面** —— 六個受測模型裡只有它跑不完。但要說準：這**只在 eval 的跑法下**被量到（連續、快速、同一個 id 連發 42 次），CLI 與 `serve` 是人在打字，不是那個形狀；**「跑不完 eval」與「不適合當預設」是兩件事**，這裡只把證據放好，翻不翻案是 demian 的判斷。
